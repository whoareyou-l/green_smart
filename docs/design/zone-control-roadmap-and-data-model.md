...
> 작성일: 2026-06-20
> 기준 버전: `v1.9.54` / Product Phase 6 baseline 완료, Crop Safety Rules C-S1B baseline 완료, Crop Interlock C-S2 baseline 완료, Model Phase M1 작기 모델 snapshot baseline 완료, UI Polish Phase P4 완료, Control Phase C19D 완료
...
v1.9.54 이후 제어 기능은 단순히 domain별 설정을 저장하는 구조가 아니라, **각 domain 내부는 Safety → Interlock → Model(AI)** 순서로 해석한다. domain 간 참조 순서는 `Crop → Environment → Irrigation → Device`다.
