...
UI Polish Phase P1 v1.9.93 방제 기록 모달 compact 기준:
...
### 8.9 P1 rendered-flow QA v1.9.93
...
Compatibility note: v1.9.93 five requested Crop Settings UI corrections 문구는 기존 current-version 계약 호환을 위해 유지하되, 실제 5개 요청사항 완료 릴리스는 v1.9.85이고 v1.9.86은 그 위의 공통 하위탭 목록 레이아웃 보정, v1.9.93은 AI 전략 패널 타입 보정이다.
...
v1.9.93 AI Strategy panel-type layout compatibility: v1.9.87에서 도입한 panel-type layout은 현재 버전에서도 유지한다.
...
v1.9.93 AI Strategy decision-oriented DOM:
...
- v1.9.93부터 `data-crop-ai-decision-flow`, `data-crop-ai-decision-flow-steps`, `data-crop-ai-flow-step` 판단 흐름 카드는 제거한다.
...
v1.9.93 AI main card unification:
...
v1.9.93 AI main card inner consistency:
...
v1.9.93 AI detail unified evidence UI:
...
## Crop Settings requested UI corrections — v1.9.93
