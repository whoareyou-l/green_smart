...
### 8.9 P1 rendered-flow QA v1.9.97
...
Compatibility note: v1.9.97 five requested Crop Settings UI corrections 문구는 기존 current-version 계약 호환을 위해 유지하되, 실제 5개 요청사항 완료 릴리스는 v1.9.85이고 v1.9.86은 그 위의 공통 하위탭 목록 레이아웃 보정, v1.9.97은 환경 제어 상태·기록형 탭 보정이며, AI 전략 패널 타입 보정은 이전 호환 기준으로 유지한다.
...
v1.9.97 AI Strategy panel-type layout compatibility: v1.9.87에서 도입한 panel-type layout은 현재 버전에서도 유지한다.

v1.9.97 AI Strategy model hierarchy restructure:
...
v1.9.97 AI Strategy decision-oriented DOM:
...
v1.9.97 AI main card unification:
...
v1.9.97 AI main card inner consistency:
...
v1.9.97 AI detail unified evidence UI:
