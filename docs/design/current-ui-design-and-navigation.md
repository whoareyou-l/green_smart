...
RB-005 Safety/Execution UI proximity (`v1.12.48`) 기준:
...
- v1.12.48 Interlock detail modal hidden hotfix: `data-crop-ai-interlock-detail-modal`은 초기 렌더에서 반드시 `display:none`이어야 하며, `오류건수` 클릭 핸들러에서만 `modal.style.display = "flex"`로 열린다. 닫기 버튼은 `modal.style.display = "none"`으로 되돌린다. `hidden` 속성과 inline `display:flex`를 동시에 두면 사이드바에서 작물 설정을 클릭하자마자 모달이 노출될 수 있으므로 금지한다.
