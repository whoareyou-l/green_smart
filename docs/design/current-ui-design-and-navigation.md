...
UI Polish Phase P1 v1.9.91 방제 기록 모달 compact 기준:
...
### 8.9 P1 rendered-flow QA v1.9.91
...
Compatibility note: v1.9.91 five requested Crop Settings UI corrections 문구는 기존 current-version 계약 호환을 위해 유지하되, 실제 5개 요청사항 완료 릴리스는 v1.9.85이고 v1.9.86은 그 위의 공통 하위탭 목록 레이아웃 보정, v1.9.91은 AI 전략 패널 타입 보정이다.
...
v1.9.91 AI Strategy panel-type layout compatibility: v1.9.87에서 도입한 panel-type layout은 현재 버전에서도 유지한다.

v1.9.91 AI Strategy model hierarchy restructure:
...
v1.9.91 AI Strategy decision-oriented DOM:
...
- `data-crop-ai-decision-flow`는 `data-crop-ai-decision-flow-steps` 안에 입력 확인 → 모델 판단 → 인터록 확인 → 운영 제안 4단계를 보여준다.
- 메인 영역 순서는 `data-crop-ai-strategy-header` → `data-crop-ai-readonly-boundary` → `data-crop-ai-decision-summary` → `data-crop-ai-decision-flow` → `data-crop-ai-interlock-summary` → `data-crop-ai-model-status-summary` → `data-crop-ai-advanced-details`이다.
...
v1.9.91 AI detail unified evidence UI:
...
## Crop Settings requested UI corrections — v1.9.91
