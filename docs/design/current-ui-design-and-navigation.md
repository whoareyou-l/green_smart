...
v1.9.54 기준 관수 페이지는 초기 진입 시 여러 API 응답마다 전체 화면을 재렌더하지 않는다.
...
### 11.6 v1.9.54 생육 AI 전략/제어 페이지 정보 구조 contract

v1.9.54 기준 작물 관리와 제어 페이지는 다음 UI 구조를 유지한다.
...
- v1.9.54 Center policy guidance: `data-center-crop-policy-guidance`, `data-center-crop-policy-reasons`, `data-center-crop-policy-next-action` marker로 정책 상태 이유와 다음 조치를 한국어로 표시한다. `fresh/stale_usable/stale_restricted/fallback_safe/rejected` 상태를 농장 운영 문구로 번역하되 실행 버튼은 추가하지 않는다.
...
### 11.7 v1.9.54 Admin/System 관리 기능 contract
