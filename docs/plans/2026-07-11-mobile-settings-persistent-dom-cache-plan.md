# v1.15.56 모바일 설정 persistent DOM cache 전환 계획
...
v1.15.56은 전체 `this.render()`는 많이 줄였지만, 모바일 하위탭 전환은 여전히 다음 구조다.
...
v1.15.56에서는 먼저 cache 기반 modal root와 marker를 도입한다.
