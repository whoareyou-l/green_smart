# v1.15.49 설정 cache prewarm 실행 계획

## 배경
v1.15.49까지 settings-admin은 기존 full render fallback 없이 `settings-shell-cache-show-hide` 경로로만 진입한다. 하지만 cache는 브라우저/WebView 메모리 안에서 생성되는 DOM cache이므로 첫 진입 전에는 비어 있다.
