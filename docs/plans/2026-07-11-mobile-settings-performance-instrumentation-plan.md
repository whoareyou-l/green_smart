# v1.15.36 모바일 설정 성능 계측 계획

## 목표

v1.15.17~21에서 모바일 설정 도메인은 persistent shell/panel cache, compact dirty patch, lazy modal cache, delegated event 구조로 전환되었다. v1.15.36의 목표는 이 구조가 실제 사용자 체감 기준을 만족하는지 확인할 수 있도록 DOM marker 기반 성능 계측을 추가하는 것이다.
