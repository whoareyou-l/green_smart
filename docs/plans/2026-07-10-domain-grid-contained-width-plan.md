# v1.15.10 도메인 화면 폭 grid-contained 맞춤 계획

## 증상

v1.15.10에서 `ha-sidebar-hidden` 모드의 content width를 `100dvw`로 바꾼 결과, 실제 화면에서 오른쪽 content가 화면 폭을 넘어가며 하단 horizontal scrollbar가 생겼다.
...
- Prod served smoke에서 v1.15.10 marker와 금지 패턴 부재를 확인한다.
