...
> **Baseline reference:** `v1.9.42` adds stage diagnosis API on top of C-S1/C-S2 skeleton; it is still calibration-driven baseline, not final agronomic readiness.
...
5. 기존 `v1.9.42`는 유지하되, 실사용 readiness로 표현하지 않는다.
6. `v1.9.42` Stage Diagnosis API는 사용자가 구현을 승인한 DB/API baseline이며, 실제 제어 자동화 결합 전에는 추가 검증 질문을 계속 진행한다.
...
| `v1.9.42` | C-S2 baseline complete | cropInterlock decision skeleton |
...
##### Implemented DB/API baseline — v1.9.42 Stage Diagnosis

`v1.9.42` adds a read-only diagnosis endpoint that combines `crop_seasons`, latest `growth_surveys`, recent `control_records/control_pesticides`, and `crop_stage_calibrations`.
