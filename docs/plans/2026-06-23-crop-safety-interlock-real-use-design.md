...
> **Baseline reference:** `v1.9.50` adds stage diagnosis API on top of C-S1/C-S2 skeleton; it is still calibration-driven baseline, not final agronomic readiness.
...
5. 기존 `v1.9.50`는 유지하되, 실사용 readiness로 표현하지 않는다.
6. `v1.9.50` Stage Diagnosis API는 사용자가 구현을 승인한 DB/API baseline이며, 실제 제어 자동화 결합 전에는 추가 검증 질문을 계속 진행한다.
...
| `v1.9.50` | C-S2 baseline complete | cropInterlock decision skeleton |
...
##### Implemented DB/API baseline — v1.9.50 Stage Diagnosis

`v1.9.50` adds a read-only diagnosis endpoint that combines `crop_seasons`, latest `growth_surveys`, recent `control_records/control_pesticides`, and `crop_stage_calibrations`.
...
##### Implemented approval/audit baseline — v1.9.50 Crop Interlock approval

`v1.9.50` adds edge-local crop interlock approval and audit persistence.
...
##### Implemented approval gate integration — v1.9.50

`v1.9.50` connects active approvals to cropInterlock target promotion gating.
...
##### Implemented center snapshot collection — v1.9.50

`v1.9.50` adds edge-to-center crop interlock snapshot sync. 센터 API는 snapshot 수집/분석 주체이며 실시간 safety/interlock 최종 판단자가 아니다.
...
##### Implemented center analytics summary — v1.9.50

`v1.9.50` adds a center-side crop interlock analytics summary endpoint. 센터 분석 API는 analytics/reporting only이며 실시간 safety/interlock 최종 판단자가 아니다.
...
##### Implemented edge/panel analytics read-only card — v1.9.50

`v1.9.50` adds an Edge HA proxy and Panel read-only card for center crop interlock analytics.
...
##### Implemented snapshot sync policy — v1.9.50
