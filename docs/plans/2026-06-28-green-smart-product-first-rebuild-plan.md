...
| `docs/design/ui-information-architecture-and-rbac.md` | 기준 버전 `v1.9.56` | 현재 구현 `v1.12.17`와 문서 기준 괴리 |
...
1. **Prod 안정성 우선:** 현재 `v1.12.17` 운영 반영 상태는 유지한다. 리빌딩 중 prod에 즉시 큰 변경을 넣지 않는다.
...
**Objective:** 현재 `v1.12.17`를 안전 기준선으로 고정하고 리빌딩 대상/보존 대상을 분리한다.
...
**Objective:** 오래된 `v1.9.56` 기준 IA/RBAC 문서를 `v1.12.17+` 기준으로 재정렬한다.
...
**Status:** `v1.12.17`에서 기준선 완료. 상세 산출물은 `docs/rebuild/frontend-decomposition-plan.md`.
...
**Status:** `v1.12.17`에서 기준선 완료. 상세 산출물은 `docs/rebuild/backend-api-decomposition-plan.md`.
...
**Status:** `v1.12.17`에서 기준선 완료. 상세 산출물은 `docs/rebuild/db-schema-rationalization-plan.md`.
...
**Status:** `v1.12.17`에서 legacy direction inventory 완료. 상세 산출물은 `docs/rebuild/legacy-direction-inventory.md`.
...
**Status:** `v1.12.17`에서 crop_cycle/currentCrop API naming boundary 완료. 상세 산출물은 `docs/rebuild/crop-cycle-api-boundary.md`.
...
**Status:** `v1.12.17`에서 target gs_permissions permission naming boundary 완료. 상세 산출물은 `docs/rebuild/rbac-permission-boundary.md`.
...
**Status:** `v1.12.17`에서 rebuild currentCrop/crop_cycle service adapter 완료. 상세 산출물은 `docs/rebuild/rebuild-current-crop-adapter.md`.
...
**Status:** `v1.12.17`에서 legacy physical DB → crop_cycle/currentCrop DTO read-only adapter 완료. 상세 산출물은 `docs/rebuild/read-only-db-adapter.md`.
...
**Status:** `v1.12.17`에서 rebuild home context API가 RS-013 read-only DB adapter service를 source로 사용하도록 연결 완료. 상세 산출물은 `docs/rebuild/rebuild-home-context-api-source-adapter.md`.
...
**Status:** `v1.12.17`에서 rebuild panel이 protected home context API를 비동기로 호출하도록 연결 완료. 상세 산출물은 `docs/rebuild/rebuild-panel-async-context-loading.md`.
...
**Status:** `v1.12.17`에서 작물상태/생육목표의 crop_cycle/currentCrop read-only UI 표시 완료. 상세 산출물은 `docs/rebuild/crop-cycle-readonly-page-slice.md`.
...
**Status:** `v1.12.17`에서 구역별 currentCrop 배정 read model 완료. 상세 산출물은 `docs/rebuild/zone-current-crop-assignment-read-model.md`.
...
**Status:** `v1.12.17`에서 생육목표 read-only projection 완료. 상세 산출물은 `docs/rebuild/growth-target-readonly-projection.md`.
