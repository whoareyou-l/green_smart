...
| `docs/design/ui-information-architecture-and-rbac.md` | 기준 버전 `v1.9.56` | 현재 구현 `v1.11.11`와 문서 기준 괴리 |
...
1. **Prod 안정성 우선:** 현재 `v1.11.11` 운영 반영 상태는 유지한다. 리빌딩 중 prod에 즉시 큰 변경을 넣지 않는다.
...
**Objective:** 현재 `v1.11.11`를 안전 기준선으로 고정하고 리빌딩 대상/보존 대상을 분리한다.
...
**Objective:** 오래된 `v1.9.56` 기준 IA/RBAC 문서를 `v1.11.11+` 기준으로 재정렬한다.
...
**Status:** `v1.11.11`에서 기준선 완료. 상세 산출물은 `docs/rebuild/frontend-decomposition-plan.md`.
...
**Status:** `v1.11.11`에서 기준선 완료. 상세 산출물은 `docs/rebuild/backend-api-decomposition-plan.md`.
...
**Status:** `v1.11.11`에서 기준선 완료. 상세 산출물은 `docs/rebuild/db-schema-rationalization-plan.md`.
...
| Slice | 범위 | Prod 위험 |
|---|---|---|
| RB-001 Admin/System shell 분리 | technical diagnostics/settings를 Admin 영역으로 이동하는 UI contract. `v1.11.11`에서 `panel/domains/admin/admin-page.js` render boundary extraction 완료 | 낮음 |
| RB-002 Panel API client adapter | `callApi` 직접 호출을 domain client로 감싸기. `v1.11.11`에서 `panel/core/api-client.js` baseline + targeted read-only call sites 연결 완료 | 낮음~중간 |
| RB-003 Crop read-only component extraction | 작물 목록/요약 read-only 렌더 분리. `v1.11.11`에서 `domains/crop/crop-readonly.js` baseline 완료 — read-only render helpers only, crop write modal/save/delete 변경 없음, DB/API 변경 없음 | 중간 |
