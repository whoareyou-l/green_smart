# v1.15.58 설정 모달 취소 후 실제 카드 미반영 원인 분석/수정 계획

## 사용자 증상

- 설정 페이지 하위탭 `온실·구역`에서 `온실 추가` 모달 진입 후 하단 `취소`를 누르면 내용카드가 요약에서 실제 카드로 전환되지 않는다.
- 이전 수정(v1.15.34/v1.15.58)이 실제 화면에 반영되지 않은 것으로 보인다.
...
- Served URL also lacks v1.15.58 markers:
  - `/green_smart_panel/rebuild/green-smart-rebuild-panel.js?v=1.15.58`
...
8. If served smoke passes but user still sees old behavior, add/verify stronger cache-busting by bumping to v1.15.58 and ensuring panel registration module URL changes in HA frontend metadata.
...
- Full tests remain passing or only skipped if no code changed after v1.15.58.
