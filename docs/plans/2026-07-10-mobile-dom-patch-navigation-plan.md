# v1.15.29 모바일 DOM patch navigation 계획

## 사용자 피드백

- v1.15.29는 대기 placeholder는 없어졌지만 여전히 실제 반응이 느리다.
- 사용자는 탭/도메인 버튼을 누르면 즉각즉각 동작하는 UX를 원한다.

## 원인

v1.15.29까지는 모바일 fast mode에서도 `setR7DomainSubtab()` 마지막에 `this.render()`를 호출했다. 이 방식은 탭 하나를 바꾸기 위해 Green Smart 전체 shell/sidebar/workspace/footer/modal HTML과 이벤트 바인딩을 다시 생성한다.
