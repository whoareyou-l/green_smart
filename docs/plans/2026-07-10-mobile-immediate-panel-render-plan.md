# v1.15.43 모바일 즉시 패널 렌더 계획

## 사용자 피드백

- v1.15.43에서 화면은 표시되지만 실제 내용 전환까지 너무 오래 걸린다.
- 사용자는 탭/도메인 버튼을 누르면 바로바로 실제 화면이 바뀌는 UX를 원한다.

## 원인

v1.15.10~v1.15.43의 2단계 placeholder/hydration 구조는 freeze를 피하려는 목적이었지만, 실제 모바일 HA WebView에서는 사용자가 `화면을 전환하는 중입니다`를 오래 보게 만들었다. 또한 설정의 `사용자·권한` 탭을 눌렀을 때 이전 `온실·구역` 맥락이 남아 보이면 진짜 화면 전환처럼 느껴지지 않는다.
