# v1.15.36 모바일 2단계 패널 hydrate 계획

## 남은 증상

- v1.15.36에서도 설정 하위탭 이동/설명·도메인 페이지 이동 시 이전 화면에서 멈춘 것처럼 보인다.
- 설정 도메인 최초 진입은 가능하지만, 큰 하위탭을 누르면 전환 프레임 자체가 막힌다.

## 재분석

v1.15.36는 모바일 fast mode에서 inactive panel 렌더는 막았지만, active panel body는 클릭 이벤트와 같은 렌더 사이클에서 즉시 만든다.
