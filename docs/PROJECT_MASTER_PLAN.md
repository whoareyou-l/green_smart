...
> **현재 우선순위 전환:** `v1.12.1` 이후 신규 기능 수직 슬라이스는 일시 중단하고, [`docs/plans/2026-06-28-green-smart-product-first-rebuild-plan.md`](plans/2026-06-28-green-smart-product-first-rebuild-plan.md)에 따라 **제품 구조 리빌딩 → 운영 스택 리빌딩** 순서로 진행한다. **기존 RB 산출물은 reference/evidence로만 사용**하며, 다음 RB 계속 진행 금지: 새 구현은 `새 master docs → 새 target architecture → 새 vertical slice scaffold` 순서의 from-scratch rebuild 기준선에서 시작한다.
