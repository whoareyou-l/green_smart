...
> 기준 버전: product code `v1.9.17`, Phase 5 Limited Auto Control + Alert Resume
...
운영 안정성 기준으로는 `C14~C16` 완료 상태이며, 이제 **제한적 현장 운영 테스트 가능** 기준에 도달했다. 다음은 C17 운영 모드/권한/확인 UX 강화다. 제품 기능 기준으로는 다음 큰 기능이 `Phase 6 — 생육 리포트와 예측`이다.
