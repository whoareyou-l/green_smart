...
> **현재 우선순위 전환:** `v1.11.9` 이후 신규 기능 수직 슬라이스는 일시 중단하고, [`docs/plans/2026-06-28-green-smart-product-first-rebuild-plan.md`](plans/2026-06-28-green-smart-product-first-rebuild-plan.md)에 따라 **제품 구조 리빌딩 → 운영 스택 리빌딩** 순서로 진행한다.
...
| `custom_components/green_smart/services/crop_service.py` | RB-006A/B Crop read-only service boundary. `CropReadActor`, `view_crop_records` permission smoke, crop seasons + growth/pest/control read delegation |
| `custom_components/green_smart/repositories/crop_repo.py` | RB-006A/B Crop read-only repository. `GET /crop/seasons`와 growth/pest/control legacy SELECT SQL 소유, write SQL 없음 |
