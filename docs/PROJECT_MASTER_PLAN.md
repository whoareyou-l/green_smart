...
> **현재 우선순위 전환:** `v1.11.10` 이후 신규 기능 수직 슬라이스는 일시 중단하고, [`docs/plans/2026-06-28-green-smart-product-first-rebuild-plan.md`](plans/2026-06-28-green-smart-product-first-rebuild-plan.md)에 따라 **제품 구조 리빌딩 → 운영 스택 리빌딩** 순서로 진행한다.
...
| `custom_components/green_smart/crop_views.py` | 현재 작기/생육/병해충/방제 API monolith이며, R3 이후 route compatibility adapter로 점진 축소. RB-006A/B/C 이후 crop seasons GET, growth/pest/control GET, 작기 create/update/delete/demolish는 service/repo로 delegate |
| `custom_components/green_smart/services/crop_service.py` | RB-006A/B/C Crop service boundary. `CropReadActor`, `CropWriteActor`, `view_crop_records`/`edit_crop_records`/`delete_crop_records` permission smoke, crop seasons + growth/pest/control read delegation, create/update/delete/demolish write helpers |
