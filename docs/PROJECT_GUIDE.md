...
> **Current baseline:** `v1.9.56` / `green_smart` Home Assistant custom integration
...
| 생육조사 | 작기별 생육 데이터 기록, 작물별 dynamic metric 저장, 생육 리포트/G-Index/작물별 수확량 예측 baseline |
...
| 실제 작물별 수확량 모델 calibration | Product Phase 6 후속 |
...
Phase 1E부터 panel에서는 JSON textarea 직접 편집 대신 세부 rule builder UI를 제공한다. 저장 구조는 여전히 `settings_json.rules[]`이며, 기존 API/DB contract는 유지한다.
...
Phase 2B semantic preset baseline:
...
Phase 2E event lifecycle baseline:
...
POST /api/green_smart/irrigation/strategy-preview
IRRIGATION_STRATEGY_COMPONENTS = IRR/EC_PH/VWC/DRYBACK
accumulatedRadiation/currentVwc/currentEc/currentPh/dryback
shotAmountL/minIntervalMin/targetEc/targetPh/targetDryback/targetDrainRate
emergencyIrrigation
targetDiff/diffCount
...
Control Phase C17 운영자 실행 확인 baseline:
...
실제 final target 실행은 운영자 확인 체크, 정확한 확인 문구, 실행 권한 role, manual/assist override 사유를 포함해야 한다. Panel에는 `운영자 실행 확인` 카드가 표시되며 `확인 후 최종값 실행` 버튼은 확인 문구가 일치할 때만 실행을 요청한다.
...
```text
실제 장비로 검증해야 하는 인터록, 운영 알고리즘, UI/운영자 UX가 가상 장치/가상 센서/시뮬레이션에서 통과하기 전에는 physical device를 연결하지 않는다.
```

Control Phase C18 현장 리허설 readiness baseline:
...
Panel에는 `현장 리허설` 카드가 표시되며 정상/강풍/강우/저온/센서 고장/차단/Fail Safe/복구 시나리오별 준비도를 확인한다. 이 단계는 실제 장비를 움직이지 않고 C14~C17 준비 요소를 묶어 체크리스트와 smoke 절차를 만든다.
...
v1.9.56 이후 Green Smart의 전략/AI 작업은 4개 모델을 바로 잇는 방식이 아니라, 다음 순서를 따른다.
