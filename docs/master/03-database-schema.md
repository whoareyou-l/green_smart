...
8. `v1.14.59` RS-017의 `currentCropAssignment`는 DB table이 아니라 read model projection이다. legacy physical schema is adapter-only이며 `sourceRowId`는 legacy row evidence로만 보존한다. No DB migration in RS-017.
