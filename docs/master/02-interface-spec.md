...
### 1.2 Rebuild currentCrop adapter boundary
...
### 1.4 Rebuild home context API source adapter
...
### 1.8 Growth target read-only projection
...
RS-018 기준 `생육목표` 화면은 `currentCropAssignment`에서 파생된 `growthTargetProjection`을 읽기 전용으로 표시한다. Projection은 `targetStageLabel`, `targetFocus`, `targetBasis`를 제공하며 목표 수정/저장/실행은 포함하지 않는다.

### 1.9 Environment impact read-only projection
...
### 1.11 Operator approval scaffold
...
RS-021 기준 `추천·실행` 화면은 추천 검토 뒤에 `operatorApprovalScaffold`를 read-only/disabled 상태로 표시한다. 작업자 승인, 안전/인터록 사전검증 전 실행은 차단된다.

### 1.12 Safety/Interlock preflight projection

```text
Safety/Interlock preflight projection
...
### 1.14 Rehearsal result review projection

```text
Rehearsal result review projection
...
### 1.16 Virtual runner dry-run result adapter

```text
Virtual runner dry-run result adapter
...
RS-026 기준 `추천·실행` 화면은 virtual runner input contract 뒤에 dry-run result adapter를 read-only로 표시한다. 결과는 `simulated_not_executed`이며 실제 runner 실행, 승인 해제, 장치 명령은 포함하지 않는다.

### 1.17 Virtual rehearsal pass/fail review projection

```text
Virtual rehearsal pass/fail review projection
...
RS-027 기준 `추천·실행` 화면은 dry-run result adapter 뒤에 pass/fail review projection을 read-only로 표시한다. 판단 권한은 `operator_review_only`이고, 실제 runner 실행·승인 해제·장치 명령은 포함하지 않는다.
...
## VS-N003 Real-time monitoring read-only scaffold interface boundary
