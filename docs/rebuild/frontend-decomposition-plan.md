...
`v1.11.8`에서 Admin/System render boundary extracted 상태가 되었다.
...
`v1.11.8`에서 Panel API client adapter baseline이 추가되었다.
...
`v1.11.8`에서 Crop read-only render helper baseline이 추가되었다.
