...
Status: current baseline for `v1.14.57`.
...
`장비 구성`은 전체 합계가 아니라 선택 구역 상태 카드다. 표시 값은 `센서, 장비, 미연결`이다.

## Device/sensor mapping

기존 `장비 생성` 카드는 `장치/센서 매핑` 진입 카드로 변경한다. 주요 버튼은 `장치/센서 매핑 열기`이며 장치/센서 매핑 팝업 모달을 연다.

## Modals and API

- 온실 생성 팝업 모달: `POST /api/green_smart/rebuild/settings/greenhouses`
- 구역 생성 팝업 모달: `POST /api/green_smart/rebuild/settings/zones`
- 장치/센서 매핑 팝업 모달: `POST /api/green_smart/rebuild/settings/device-sensor-mappings`
