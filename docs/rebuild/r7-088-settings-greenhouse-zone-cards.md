...
## CDB card grammar hotfix in v1.15.49
...
action row: 3 two-button cards including irrigation group create
  - 온실 생성
  - 구역 생성
  - 관수그룹 생성
...
- The action row keeps `data-r7-cdb-layout-row="actions"` and three `renderR7CdbButtonTwoCard()` cards: greenhouse create, zone create, and irrigation-group create/list.
...
- `data-r7-settings-greenhouse-summary-card="greenhouse-basic-info"` — 온실 정보
- `data-r7-settings-greenhouse-summary-card="zone-basic-info"` — 구역 정보; historical wording: 구역 기본 정보 / 구역 구성
- `data-r7-settings-greenhouse-summary-card="equipment-composition"` — 관수그룹 정보

Action cards:

- `data-r7-settings-create-card="settings-greenhouse-create"`
- `data-r7-settings-create-card="settings-zone-create"`
- `data-r7-settings-check-card="settings-irrigation-group-create"`
...
온실 생성/구역 생성 버튼은 공통 모달 shell과 API 연결 상태를 유지한다. 온실·구역 하위탭의 관수그룹 카드는 `관수그룹 생성` 2버튼 카드로 그룹 생성 모달과 그룹 목록 모달을 연다. 이 문서는 화면 문법과 marker contract를 다루며, DB mutation 세부 정책은 `r7-098-settings-greenhouse-zone-real-db-api.md`에서 관리한다.
