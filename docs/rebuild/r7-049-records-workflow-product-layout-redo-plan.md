...
**Redo required:** Yes. The previous `v1.15.39` implementation must be treated as baseline evidence, not as the final product layout.
...
Therefore `v1.15.39` must be redone as `v1.15.39` with a real product workflow layout.
...
HA check_config
restart HA
served-source smoke: v1.15.39 + R7-049 markers
