...
**Target version:** v1.14.50
**Scope:** `작물 운영 > 기록·작업` subtab only
**Redo required:** Yes. The previous `v1.14.50` implementation must be treated as baseline evidence, not as the final product layout.
...
Therefore `v1.14.50` must be redone as `v1.14.50` with a real product workflow layout.
...
1. Version surfaces are `1.14.50`.
...
HA check_config
restart HA
served-source smoke: v1.14.50 + R7-049 markers
