...
**Target version:** v1.14.8
**Scope:** `작물 운영 > 기록·작업` subtab only
**Redo required:** Yes. The previous `v1.14.8` implementation must be treated as baseline evidence, not as the final product layout.
...
Therefore `v1.14.8` must be redone as `v1.14.8` with a real product workflow layout.
...
HA check_config
restart HA
served-source smoke: v1.14.8 + R7-049 markers
