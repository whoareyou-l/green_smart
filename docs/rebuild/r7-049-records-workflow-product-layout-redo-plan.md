...
**Redo required:** Yes. The previous `v1.13.1` implementation must be treated as baseline evidence, not as the final product layout.
...
Therefore `v1.13.1` must be redone as `v1.13.1` with a real product workflow layout.
...
HA check_config
restart HA
served-source smoke: v1.13.1 + R7-049 markers
