// Green Smart — Modern SaaS greenhouse dashboard  v1.10.21
...
<div id="app">
  ...
  _renderGrowthReportCard() {
    ...
    const environmentRiskScore = maxRiskScore(riskFactorPrediction.environmentStress || {});
    const irrigationRiskScore = maxRiskScore(riskFactorPrediction.irrigationNutrientStress || {});
    const pestDiseaseRiskScore = maxRiskScore(riskFactorPrediction.pestDiseaseRisk || {});
    ...
    const growthStateLabel = `balance ${String(currentGrowthBalance.balanceScore ?? 0)} · directionCode ${String(currentGrowthBalance.directionCode ?? 9)}`;
    ...
    return `<section class="gs-card" data-growth-report-card data-crop-ai-evidence-panel style="padding:14px;margin-bottom:14px;background:#fbfefb;border:1px solid #e3f1e5;">
      ...
      <section data-crop-ai-main-card="crop-status" data-crop-ai-decision-summary data-crop-ai-primary-summary data-crop-ai-summary-stack data-crop-ai-crop-summary style="background:#fff;border:1px solid #cfe8d8;border-radius:16px;padding:14px;margin-bottom:12px;box-shadow:0 6px 18px rgba(64,117,78,0.08);">
        <div data-crop-ai-main-card-header data-crop-ai-section-heading style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;margin-bottom:10px;"><div><div style="font-size:15px;font-weight:900;color:#24323F;">작물 요약</div><div style="font-size:11px;color:#7a9780;margin-top:3px;">이번 주 작물 판단 요약 · 농장주/직원이 먼저 확인할 작물 상태입니다.</div></div><span style="font-size:10px;font-weight:900;border-radius:999px;padding:4px 9px;background:#f7fbff;color:#6d8799;border:1px solid #dbeaf8;">crop</span></div>
        <div data-crop-ai-main-card-body style="display:flex;flex-direction:column;gap:9px;">
          <div data-crop-ai-main-metric-grid data-crop-ai-primary-metric-grid style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;">
            <div data-crop-ai-summary-stage data-crop-ai-main-metric data-crop-ai-primary-gl-index style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">작물단계</div><b data-crop-ai-main-metric-value style="font-size:18px;color:#24323F;">${this._esc(stageDiagnosis.stageLabel || predictedStage7d.stageLabel || '미확정')}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">G/L-Index · 7일 ${this._esc(predictedStage7d.stageLabel || '-')}</div></div>
            <div data-crop-ai-summary-growth-state data-crop-growth-state-numeric-card data-crop-ai-main-metric data-crop-ai-growth-state-score style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">작물상태</div><b data-crop-ai-main-metric-value data-crop-growth-state-balance-score style="font-size:18px;color:#24323F;">${this._esc(String(currentGrowthBalance.balanceScore ?? 0))}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">${this._esc(growthStateLabel)}</div></div>
            <div data-crop-ai-summary-environment-risk data-crop-risk-factor-numeric-card data-crop-ai-main-metric style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">환경리스크</div><b data-crop-ai-main-metric-value data-crop-risk-factor-score style="font-size:18px;color:#24323F;">${this._esc(String(environmentRiskScore))}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">온도/VPD/습도/CO₂/광량</div></div>
            <div data-crop-ai-summary-irrigation-risk data-crop-ai-primary-yield-prediction data-crop-ai-main-metric style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">관수리스크</div><b data-crop-ai-main-metric-value style="font-size:18px;color:#24323F;">${this._esc(String(irrigationRiskScore))}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">EC/pH/dry-back/배액 · 수확량 예측은 상세 근거</div></div>
            <div data-crop-ai-summary-pest-risk data-crop-ai-main-metric data-crop-ai-primary-pest-risk style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">병충해리스크</div><b data-crop-ai-main-metric-value style="font-size:18px;color:${pestDiseaseRiskScore >= 0.7 ? '#c0392b' : pestDiseaseRiskScore >= 0.4 ? '#f39c12' : '#51AE60'};">${this._esc(String(pestDiseaseRiskScore))}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">병해 위험도 · ${this._esc(riskLabel)}</div></div>
          </div>
          ...
          <div data-crop-ai-main-action-row style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;"><span style="font-size:10px;color:#7a9780;font-weight:800;background:#f5faf6;border:1px solid #e2f1e7;border-radius:999px;padding:4px 8px;">작물 요약</span><span style="font-size:10px;color:#7a9780;font-weight:800;background:#f5faf6;border:1px solid #e2f1e7;border-radius:999px;padding:4px 8px;">상세 근거는 모델 상태 카드</span></div>
        </div>
      ...
      <section data-crop-ai-main-card="interlock-status" data-crop-ai-safety-interlock-summary data-crop-ai-support-status-summary data-crop-ai-interlock-summary data-crop-interlock-card style="background:#fff;border:1px solid ${cropInterlock.cropInterlockBlocked ? '#f3c8c8' : '#cfe8d8'};border-radius:16px;padding:14px;margin-bottom:12px;box-shadow:0 6px 18px rgba(64,117,78,0.08);">
        ...
        <div data-crop-ai-main-card-body style="display:flex;flex-direction:column;gap:9px;">
          <div data-crop-ai-main-metric-grid data-crop-ai-interlock-grid style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;">
            ...
            <div data-crop-ai-summary-error-count data-crop-ai-main-metric style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">오류건수</div><b data-crop-ai-main-metric-value style="font-size:15px;color:${safetyIssueCount ? '#c0392b' : '#51AE60'};">${this._esc(String(safetyIssueCount))}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">인터록/승인/운영 경고 합계</div></div>
          </div>
          <div data-crop-ai-main-note style="background:#f8fbf9;border:1px solid #e2f1e7;border-radius:12px;padding:10px;font-size:11px;color:#7a9780;line-height:1.55;"><b>안전/인터록 확인</b> 안전상태 · 인터록 상태 · 오류건수를 먼저 확인합니다.<span data-crop-ai-target-promotion-status data-crop-ai-auto-execution-status style="display:none;">legacy target promotion · 자동 실행 · cropInterlockReasons · cropInterlockActions · require_harvest_safety_clearance markers retained</span></div>
