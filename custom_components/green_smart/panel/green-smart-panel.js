// Green Smart — Modern SaaS greenhouse dashboard  v1.10.20
...
<div id="app">
  ...
  _renderCropBasicOverviewCard() {
    ...
    const seasons = this._cropSeasons || [];
    ...
    const selectedVariety = selected?.variety ? ` · ${this._esc(selected.variety)}` : "";
    const selectedZone = selected ? this._seasonZoneLabel(selected) : "구역 미지정";
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;align-items:stretch;">
          <div data-crop-basic-selected-season data-crop-basic-latest-season style="min-width:0;">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;">
              ...
              <span style="font-size:11px;color:#4a6741;background:#eef8ef;border-radius:999px;padding:4px 9px;">${this._esc(selectedZone)}</span>
              <span style="font-size:11px;color:#4a6741;background:#eef8ef;border-radius:999px;padding:4px 9px;">${this._esc(selectedMethod)}</span>
            </div>
  ...
  _renderGrowthReportCard() {
    ...
    return `<section class="gs-card" data-growth-report-card data-crop-ai-evidence-panel style="padding:14px;margin-bottom:14px;background:#fbfefb;border:1px solid #e3f1e5;">
      ...
      <section data-crop-ai-main-card="crop-status" data-crop-ai-decision-summary data-crop-ai-primary-summary data-crop-ai-summary-stack data-crop-ai-model-pipeline-summary style="background:#fff;border:1px solid #cfe8d8;border-radius:16px;padding:14px;margin-bottom:12px;box-shadow:0 6px 18px rgba(64,117,78,0.08);">
        <div data-crop-ai-main-card-header data-crop-ai-section-heading style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;margin-bottom:10px;"><div><div style="font-size:15px;font-weight:900;color:#24323F;">작물 모델 파이프라인</div><div style="font-size:11px;color:#7a9780;margin-top:3px;">이번 주 작물 판단 요약 · 완성된 5개 작물 모델을 순서대로 확인하고, 마지막 조치 추천은 검토 요청까지만 표시합니다.</div></div><span style="font-size:10px;font-weight:900;border-radius:999px;padding:4px 9px;background:#f7fbff;color:#6d8799;border:1px solid #dbeaf8;">crop models</span></div>
        <div data-crop-ai-main-card-body style="display:flex;flex-direction:column;gap:9px;">
          <div data-crop-ai-main-metric-grid data-crop-ai-primary-metric-grid style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;">
            <div data-crop-ai-model-pipeline-step="stage-prediction" data-crop-ai-main-metric data-crop-ai-primary-gl-index style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">1. 생육단계 예측</div><b data-crop-ai-main-metric-value style="font-size:18px;color:#24323F;">${this._esc(stageDiagnosis.stageLabel || predictedStage7d.stageLabel || '미확정')}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">G/L-Index · 7일 ${this._esc(predictedStage7d.stageLabel || '-')} · confidenceScore ${this._esc(String(stagePredictionScore.confidenceScore ?? '-'))}</div></div>
            <div data-crop-ai-model-pipeline-step="growth-state-prediction" data-crop-growth-state-numeric-card data-crop-ai-main-metric data-crop-ai-growth-state-score style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">2. 생육상태 예측</div><b data-crop-ai-main-metric-value data-crop-growth-state-balance-score style="font-size:18px;color:#24323F;">${this._esc(String(currentGrowthBalance.balanceScore ?? 0))}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">7일 ${this._esc(String(predictedGrowthBalance7d.balanceScore ?? 0))} · directionCode ${this._esc(String(currentGrowthBalance.directionCode ?? 9))}</div></div>
            <div data-crop-ai-model-pipeline-step="risk-factor-prediction" data-crop-risk-factor-numeric-card data-crop-ai-main-metric data-crop-ai-primary-pest-risk style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">3. 위험요소 예측</div><b data-crop-ai-main-metric-value data-crop-risk-factor-score style="font-size:18px;color:#24323F;">${this._esc(String(aggregateRiskFactor.score ?? 0))}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">병해 위험도 · bandCode <span data-crop-risk-factor-band-code>${this._esc(String(aggregateRiskFactor.bandCode ?? 9))}</span> · highest riskCode ${this._esc(String(highestRiskFactor.riskCode ?? 0))} · trendCode <span data-crop-risk-factor-trend-code>${this._esc(String(aggregateRiskFactor.trendCode ?? 9))}</span></div></div>
            <div data-crop-ai-model-pipeline-step="integrated-diagnosis" data-crop-integrated-diagnosis-card data-crop-ai-main-metric style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">4. 통합 작물 진단</div><b data-crop-ai-main-metric-value data-crop-diagnosis-source-sink-gap style="font-size:18px;color:#24323F;">${this._esc(String(sourceSinkDiagnosis.sourceSinkGapScore ?? 0))}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">transitionNeedCode <span data-crop-diagnosis-transition-need-code>${this._esc(String(transitionDiagnosis.transitionNeedCode ?? 9))}</span></div></div>
            <div data-crop-ai-model-pipeline-step="action-recommendation" data-crop-action-recommendation-card data-crop-ai-primary-yield-prediction data-crop-ai-main-metric style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">5. 조치 추천 요청</div><b data-crop-ai-main-metric-value style="font-size:18px;color:#24323F;">${this._esc(String(Object.keys(workReviewRequests).length + Object.keys(modelReviewRequests).length))}건</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">request only · targetCandidateAuthorityCode 0 · 수확량 예측은 상세 근거에서 확인</div></div>
          </div>
          ...
          <div data-crop-ai-main-action-row style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;"><span style="font-size:10px;color:#7a9780;font-weight:800;background:#f5faf6;border:1px solid #e2f1e7;border-radius:999px;padding:4px 8px;">농장주/직원용 요약 우선</span><span style="font-size:10px;color:#7a9780;font-weight:800;background:#f5faf6;border:1px solid #e2f1e7;border-radius:999px;padding:4px 8px;">상세 근거는 접힘 영역</span></div>
        </div>
        <div data-crop-ai-main-card-chip-group style="font-size:10px;color:#9aae9d;margin-top:8px;line-height:1.45;">작물 모델 1차 세트 완료 · read-only · 자동 실행 없음 · 최종 setpoint 계산 없음</div>
      </section>
      <section data-crop-ai-main-card="review-requests" data-crop-ai-review-request-summary style="background:#fff;border:1px solid #cfe8d8;border-radius:16px;padding:14px;margin-bottom:12px;box-shadow:0 6px 18px rgba(64,117,78,0.08);">
        <div data-crop-ai-main-card-header style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;margin-bottom:10px;"><div><div style="font-size:15px;font-weight:900;color:#24323F;">검토 요청 요약</div><div style="font-size:11px;color:#7a9780;margin-top:3px;">조치 추천 모델이 만든 작업 검토/후속 모델 검토 요청입니다. 실행 없음 · work order 생성 없음.</div></div><span style="font-size:10px;font-weight:900;border-radius:999px;padding:4px 9px;background:#f7fbff;color:#6d8799;border:1px solid #dbeaf8;">request only</span></div>
        <div data-crop-ai-main-card-body style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;">
        ...
        <div data-crop-ai-main-card-chip-group style="font-size:10px;color:#9aae9d;margin-top:8px;line-height:1.45;">read-only · no target value · no work order · no execution · 자동 실행 없음</div>
      </section>
      <section data-crop-ai-main-card="interlock-status" data-crop-ai-support-status-summary data-crop-ai-interlock-summary data-crop-interlock-card style="background:#fff;border:1px solid ${cropInterlock.cropInterlockBlocked ? '#f3c8c8' : '#cfe8d8'};border-radius:16px;padding:14px;margin-bottom:12px;box-shadow:0 6px 18px rgba(64,117,78,0.08);">
        <div data-crop-ai-main-card-header style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;margin-bottom:10px;"><div><div style="font-size:15px;font-weight:900;color:#24323F;">인터록/모델 운영 상태</div><div style="font-size:11px;color:#7a9780;margin-top:3px;">작물 모델 결과를 운영에 참고하기 전 확인해야 하는 지원 상태입니다. 작물 인터록 / 인터록 상태 요약을 함께 확인합니다.</div></div><span style="font-size:10px;font-weight:900;border-radius:999px;padding:4px 9px;background:#f7fbff;color:#6d8799;border:1px solid #dbeaf8;">support</span></div>
        <div data-crop-ai-main-card-body style="display:flex;flex-direction:column;gap:9px;">
          <div data-crop-ai-main-metric-grid data-crop-ai-interlock-grid style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;">
            <div data-crop-ai-main-metric data-crop-ai-interlock-status style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">인터록 상태</div><b data-crop-ai-main-metric-value style="font-size:15px;color:${cropInterlock.cropInterlockBlocked ? '#c0392b' : '#51AE60'};">${this._esc(cropInterlock.cropInterlockStatus || 'clear')}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">차단/승인 필요 상태</div></div>
            <div data-crop-ai-main-metric data-crop-ai-target-promotion-status style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">목표 승격</div><b data-crop-ai-main-metric-value style="font-size:15px;color:${cropInterlock.blockTargetPromotion ? '#c0392b' : '#51AE60'};">${cropInterlock.blockTargetPromotion ? '차단' : '허용'}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">target promotion</div></div>
            <div data-crop-ai-main-metric data-crop-ai-auto-execution-status style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">자동 실행</div><b data-crop-ai-main-metric-value style="font-size:15px;color:${cropInterlock.blockAutoExecution ? '#c0392b' : '#51AE60'};">${cropInterlock.blockAutoExecution ? '차단' : '허용'}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">현장 Edge 최종 판단</div></div>
          </div>
          <div data-crop-ai-main-note style="background:#f8fbf9;border:1px solid #e2f1e7;border-radius:12px;padding:10px;font-size:11px;color:#7a9780;line-height:1.55;"><b>cropInterlockReasons</b> ${interlockReasons.length ? interlockReasons.map(r => this._esc(r)).join(' · ') : '없음'} · <b>cropInterlockActions</b> ${interlockActions.length ? interlockActions.map(a => this._esc(a)).join(' · ') : '없음'} · require_harvest_safety_clearance</div>
          <div data-crop-interlock-approval-gate style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;">
          ...
          <div data-crop-ai-main-action-row data-crop-ai-interlock-actions style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
      ...
      <section data-crop-ai-main-card="model-status" data-crop-ai-model-status-summary data-crop-ai-expanded-model-summary style="background:#fff;border:1px solid #cfe8d8;border-radius:16px;padding:14px;margin-bottom:12px;box-shadow:0 6px 18px rgba(64,117,78,0.08);">
        <div data-crop-ai-main-card-header data-crop-ai-section-heading style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;margin-bottom:10px;"><div><div style="font-size:15px;font-weight:900;color:#24323F;">모델 상태 요약</div><div style="font-size:11px;color:#7a9780;margin-top:3px;">AI 판단에 사용된 입력 완성도와 모델 준비 상태입니다.</div></div><span style="font-size:10px;font-weight:900;border-radius:999px;padding:4px 9px;background:#f7fbff;color:#6d8799;border:1px solid #dbeaf8;">main</span></div>
        <div data-crop-ai-main-card-body style="display:flex;flex-direction:column;gap:9px;">
          <div data-crop-ai-main-metric-grid data-crop-ai-model-status-grid style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;">
            ...
            <div data-crop-ai-main-metric data-crop-ai-stage-status style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">생육단계/예측</div><b data-crop-ai-main-metric-value style="font-size:15px;color:#24323F;">${this._esc(stageDiagnosis.stageLabel || predictedStage7d.stageLabel || '미확정')}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">검증 ${this._esc(validationStatusLabel)}</div></div>
            <div data-crop-ai-main-metric data-crop-ai-risk-status style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">리스크</div><b data-crop-ai-main-metric-value style="font-size:15px;color:${pestRisk.level === 'high' ? '#c0392b' : pestRisk.level === 'medium' ? '#f39c12' : '#51AE60'};">${riskLabel}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">인터록 ${this._esc(cropInterlock.cropInterlockStatus || 'clear')}</div></div>
        ...
        <div data-crop-ai-main-card-chip-group style="font-size:10px;color:#9aae9d;margin-top:8px;line-height:1.45;">read-only model status · automatic ML deployment 없음</div>
      </section>
        ...
        return `
          <div data-entry="${idx}" data-control-pesticide-entry
            ...
            <div data-control-dose-grid data-control-usage-row class="pop-field-row" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-bottom:10px;">
              ...
              <div class="pop-field">
                <label>물 사용량</label>
                <input type="number" data-water-amount-input="${idx}" value="${this._esc(e.waterAmount)}" placeholder="예) 500" min="0" step="0.1">
              </div>
            </div>
            <div class="pop-field-row">
              <div class="pop-field">
                <label>사용 면적(㎡)</label>
                <input type="number" data-treatment-area-input="${idx}" value="${this._esc(e.treatmentAreaM2)}" placeholder="작기+처리범위로 자동" min="0" step="0.1">
              </div>
              <div class="pop-field">
                <label>평당 사용량</label>
                <input type="text" data-pyeong-amount-output="${idx}" value="${this._esc(e.perPyeongUsage)}" placeholder="평당 사용량 자동 계산" readonly>
              </div>
            </div>
            <div class="pop-field">
              <label>사용량</label>
              <input type="text" data-amount-input="${idx}" value="${this._esc(e.amount)}" placeholder="약제/물/면적 입력 시 자동 요약">
