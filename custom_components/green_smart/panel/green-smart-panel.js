// Green Smart — Modern SaaS greenhouse dashboard  v1.10.22
...
<div id="app">
  ...
  _renderGrowthReportCard() {
    ...
    const environmentSummaryLabel = environmentRiskLabelMap[environmentRiskKey] || (environmentRiskScore > 0 ? "환경 변동" : "안정");
    const irrigationSummaryLabel = irrigationRiskLabelMap[irrigationRiskKey] || (irrigationRiskScore > 0 ? "관수 변동" : "안정");
    ...
    const growthStateText = growthDirectionCode > 0
      ...
      : "균형 생장";
    const growthDirectionEmoji = growthDirectionCode > 0 ? "🍅" : growthDirectionCode < 0 ? "🌿" : "⚖️";
      ...
      <section data-crop-ai-main-card="crop-status" data-crop-ai-decision-summary data-crop-ai-primary-summary data-crop-ai-summary-stack data-crop-ai-crop-summary style="background:#fff;border:1px solid #cfe8d8;border-radius:16px;padding:14px;margin-bottom:12px;box-shadow:0 6px 18px rgba(64,117,78,0.08);">
        ...
        <div data-crop-ai-main-card-body style="display:flex;flex-direction:column;gap:9px;">
          <div data-crop-ai-main-metric-grid data-crop-ai-primary-metric-grid style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;">
            ...
            <div data-crop-ai-summary-environment-risk data-crop-risk-factor-numeric-card data-crop-ai-main-metric style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">환경요약</div><b data-crop-ai-summary-environment-label data-crop-ai-main-metric-value data-crop-risk-factor-score style="font-size:18px;color:#24323F;">${this._esc(environmentSummaryLabel)}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">고온 · 저온 · 온도급변 기준 / 스코어 <span data-crop-ai-summary-environment-score>${this._esc(String(environmentRiskScore))}</span> · 신뢰 <span data-crop-ai-summary-environment-confidence>${this._esc(String(environmentRiskItem.confidenceScore ?? aggregateRiskFactor.confidenceScore ?? 0))}</span></div></div>
            <div data-crop-ai-summary-irrigation-risk data-crop-ai-primary-yield-prediction data-crop-ai-main-metric style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">관수요약</div><b data-crop-ai-summary-irrigation-label data-crop-ai-main-metric-value style="font-size:18px;color:#24323F;">${this._esc(irrigationSummaryLabel)}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">높은 EC · 과관수 기준 / 스코어 <span data-crop-ai-summary-irrigation-score>${this._esc(String(irrigationRiskScore))}</span> · 신뢰 <span data-crop-ai-summary-irrigation-confidence>${this._esc(String(irrigationRiskItem.confidenceScore ?? aggregateRiskFactor.confidenceScore ?? 0))}</span></div></div>
            <div data-crop-ai-summary-pest-risk data-crop-ai-main-metric data-crop-ai-primary-pest-risk style="background:#f8fbf9;border-radius:12px;padding:10px;border:1px solid #e2f1e7;"><div data-crop-ai-main-metric-label style="font-size:11px;color:#5f7f70;font-weight:900;">병충해요약</div><b data-crop-ai-summary-pest-score data-crop-ai-main-metric-value style="font-size:18px;color:${pestDiseaseRiskScore >= 0.7 ? '#c0392b' : pestDiseaseRiskScore >= 0.4 ? '#f39c12' : '#51AE60'};">${this._esc(String(pestDiseaseRiskScore))}</b><div data-crop-ai-main-metric-help style="font-size:10px;color:#8aa091;margin-top:3px;">신뢰 <span data-crop-ai-summary-pest-confidence>${this._esc(String(pestDiseaseRiskItem.confidenceScore ?? aggregateRiskFactor.confidenceScore ?? 0))}</span></div></div>
          </div>
