      ...
      : tabKey === "crop-cycle-objects"
        ...
        : tabKey === "device-sensor-mapping"
          ...
          : tabKey === "users-permissions"
            ? `<section data-r7-settings-users-permissions data-r7-settings-users-permissions-image-layout="true" style="display:grid;grid-template-columns:minmax(320px,1.12fr) minmax(320px,1.18fr);gap:12px;align-items:start;">
                <article data-r7-settings-users-card="user-list" style="border:1px solid #dcebe0;border-radius:18px;background:#fff;padding:14px;display:grid;gap:12px;box-shadow:0 8px 22px rgba(31,67,45,.04);">
                  <strong style="font-size:15px;color:#24323f;">사용자 목록</strong>
                  <div style="display:grid;grid-template-columns:1.05fr .9fr .75fr .9fr 1fr 24px;gap:8px;align-items:center;color:#52675b;font-size:11px;font-weight:900;border-bottom:1px solid #edf4ef;padding-bottom:8px;"><span>사용자</span><span>역할</span><span>상태</span><span>최근 활동</span><span>권한 요약</span><span></span></div>
                  ${[["A","admin","admin","활성","방금 전","전체 설정","#43ad5e","#e8f7ee"],["O","owner01","farm_owner","활성","오늘 09:20","승인 · 전략","#43ad5e","#e8f7ee"],["S","staff01","farm_staff","초대 대기","-","기록 · 모니터링","#f0a840","#fff4d6"]].map(([initial,user,role,status,activity,summary,color,bg]) => `<div data-r7-settings-user-row="${user}" style="display:grid;grid-template-columns:1.05fr .9fr .75fr .9fr 1fr 24px;gap:8px;align-items:center;font-size:12px;color:#24323f;border-bottom:1px solid #f1f6f2;padding:4px 0 7px;"><span style="display:flex;align-items:center;gap:8px;font-weight:900;"><i style="width:18px;height:18px;border-radius:999px;background:${color};color:#fff;display:inline-grid;place-items:center;font-size:10px;font-style:normal;">${initial}</i>${user}</span><span data-r7-settings-user-role-card="${role}" style="justify-self:start;border:1px solid #dcebe0;border-radius:999px;background:#f4fbf5;color:#31523b;padding:3px 7px;font-size:11px;font-weight:900;">${role}</span><span style="justify-self:start;border-radius:999px;background:${bg};color:${color};padding:3px 7px;font-size:11px;font-weight:900;">${status}</span><span style="color:#52675b;">${activity}</span><span>${summary}</span><span style="color:#78927f;font-weight:900;">⋮</span></div>`).join("")}
                  <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding-top:2px;"><button type="button" data-r7-settings-users-action="invite" style="border:1px solid #badcc8;border-radius:8px;background:#fff;color:#31523b;padding:9px 10px;font-size:12px;font-weight:950;">👥 사용자 초대</button><button type="button" data-r7-settings-users-action="role-change" style="border:1px solid #badcc8;border-radius:8px;background:#fff;color:#31523b;padding:9px 10px;font-size:12px;font-weight:950;">👥 역할 변경</button></div>
                </article>
                <article data-r7-settings-users-card="permission-matrix" style="border:1px solid #dcebe0;border-radius:18px;background:#fff;padding:14px;display:grid;gap:9px;box-shadow:0 8px 22px rgba(31,67,45,.04);">
                  <strong style="font-size:15px;color:#24323f;">권한 버킷 매트릭스</strong>
                  <div style="display:grid;grid-template-columns:1fr repeat(3,1fr);border:1px solid #edf4ef;border-radius:12px;overflow:hidden;font-size:12px;color:#24323f;">
                    ${[["권한 버킷","admin","farm_owner","farm_staff"],["조회","✅ 허용","✅ 허용","✅ 허용"],["기록","✅ 허용","✅ 허용","✅ 허용"],["전략","✅ 허용","✅ 허용","👁️ 읽기 전용"],["실행","✅ 허용","✅ 허용","🕘 요청 후 실행"],["안전","✅ 허용","🛡️ 확인","👁️ 읽기 전용"],["고급설정","✅ 허용","🛡️ 확인","🔒 없음"]].map((row, rowIndex) => row.map((cell, colIndex) => `<span ${rowIndex === 0 && colIndex > 0 ? `data-r7-settings-permission-role="${cell}"` : ""} ${colIndex === 0 && rowIndex > 0 ? `data-r7-settings-permission-bucket="${cell}"` : ""} style="border-right:${colIndex === 3 ? '0' : '1px solid #edf4ef'};border-bottom:${rowIndex === 6 ? '0' : '1px solid #edf4ef'};padding:7px 8px;font-weight:${rowIndex === 0 || colIndex === 0 ? '950' : '800'};background:${rowIndex === 0 ? '#fbfdfb' : '#fff'};color:${cell.includes('없음') ? '#5d6871' : cell.includes('확인') || cell.includes('요청') ? '#9a6b10' : cell.includes('읽기') ? '#326aa5' : '#31523b'};">${cell}</span>`).join("")).join("")}
                  </div>
                  <small style="color:#78927f;font-size:11px;">ⓘ UI 표시는 backend enforcement를 분리해서 보여줍니다.</small><span data-r7-settings-permission-bucket-card style="display:none;">조회 · 기록 · 전략 · 실행 · 안전 · 고급설정</span>
                </article>
                <article data-r7-settings-users-card="approval-queue" style="border:1px solid #dcebe0;border-radius:18px;background:#fff;padding:14px;display:grid;gap:10px;">
                  <strong style="font-size:15px;color:#24323f;">승인 필요 작업</strong>
                  ${[["자동제어 활성화","farm_owner 승인"],["안전 리밋 변경","farm_owner 승인"],["HA entity mapping 변경","admin 승인"]].map(([label, approver]) => `<div data-r7-settings-approval-row="${label}" style="display:flex;align-items:center;justify-content:space-between;gap:10px;font-size:12px;color:#24323f;padding:4px 0;"><span>⌛ ${label}</span><span style="border:1px solid #dcebe0;border-radius:999px;background:#fbfdfb;color:#52675b;padding:4px 8px;font-size:11px;font-weight:900;">${approver}</span></div>`).join("")}
                  <button type="button" data-r7-settings-users-action="approval-all" style="border:1px solid #badcc8;border-radius:8px;background:#fff;color:#31523b;padding:9px 10px;font-size:12px;font-weight:950;">모든 승인 요청 확인 〉</button>
                </article>
                <article data-r7-settings-users-card="audit-log" style="border:1px solid #dcebe0;border-radius:18px;background:#fff;padding:14px;display:grid;gap:10px;">
                  <strong style="font-size:15px;color:#24323f;">감사 로그</strong>
                  ${[["A","admin","역할 변경: staff01 → farm_staff","2026-06-30 09:12"],["O","owner01","안전 정책 승인: 온도 고한 28→30℃","2026-06-30 08:45"],["S","staff01","생육조사 작성: A구역 · 상추","2026-06-30 08:10"]].map(([initial,user,summary,time]) => `<div data-r7-settings-audit-row="${user}" style="display:grid;grid-template-columns:auto 1fr auto;gap:8px;align-items:center;font-size:12px;color:#24323f;"><i style="width:20px;height:20px;border-radius:999px;background:#43ad5e;color:#fff;display:inline-grid;place-items:center;font-size:10px;font-style:normal;">${initial}</i><span><b>${user}</b><br><small style="color:#52675b;">${summary}</small></span><time style="color:#52675b;font-size:11px;">${time}</time></div>`).join("")}
                  <button type="button" data-r7-settings-users-action="audit-all" style="border:1px solid #badcc8;border-radius:8px;background:#fff;color:#31523b;padding:9px 10px;font-size:12px;font-weight:950;">전체 감사 로그 보기 ↗</button>
                </article>
              </section>`
