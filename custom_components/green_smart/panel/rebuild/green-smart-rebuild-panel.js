...
class GreenSmartRebuildPanel extends HTMLElement {
  ...
  renderR7SettingsDeviceSensorMappingSubtab(zones = []) {
    ...
    const actions = [
      ...
      actionCard({ kind: "group-add", title: "그룹 추가", icon: "mdi:view-grid-plus-outline", primary: "구역 FK 필수", note: "그룹 생성 단계에서 구역 정보를 외래키로 저장합니다.", addLabel: "그룹 추가", addAttr: 'data-r7-settings-device-group-button data-r7-settings-device-process="group-create-zone-fk" data-r7-settings-device-group-zone-fk="required"', shortcutLabel: "그룹-장치 연결", shortcutAttr: 'data-r7-settings-device-group-link-button data-r7-settings-device-process="group-device-link" data-r7-settings-device-group-link-stage="device-to-group"' }),
    ].join("");
    ...
    const processNote = `<section data-r7-settings-device-process-summary style="border:1px solid #e2eee5;border-radius:16px;background:#fbfdfb;padding:12px;color:#31523b;font-size:12px;font-weight:850;line-height:1.6;">1. 장치 추가 → 2. 그룹 추가 → 3. 그룹에 장치 연결 · 그룹 생성 단계에서 구역 정보를 외래키로 저장 · 하나의 장치를 여러 그룹에 연결할 수 있습니다</section>`;
    return `<section data-r7-settings-device-sensor-mapping data-r7-settings-device-mapping-layout="device-group-error-device-list" style="display:grid;gap:12px;"><div data-r7-settings-device-summary-grid style="display:grid;grid-template-columns:repeat(3,minmax(210px,1fr));gap:12px;">${deviceCard}${groupCard}${errorCard}</div><div data-r7-settings-device-action-row style="display:grid;grid-template-columns:repeat(2,minmax(210px,1fr));gap:12px;">${actions}</div>${processNote}${deviceList}</section>`;
  }
