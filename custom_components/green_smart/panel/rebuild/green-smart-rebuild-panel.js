...
class GreenSmartRebuildPanel extends HTMLElement {
  ...
  renderR7SettingsSystemActionModal() {
    ...
    if (modal.kind === "errors") {
      ...
      const rowHtml = rows.map((row) => this.renderR7CdaCompactListRow({ selected: row === selected, attrs: `data-r7-settings-system-errors-row="${row.scope}" data-r7-settings-system-errors-list-item-button="${row.scope}"`, columns: [`<b>${row.scope}</b>`, `<span>${row.status}</span>`, `<span>${row.count || 0}건</span>`, `<span>${(row.hints || []).slice(0, 1).join(" · ")}</span>`] })).join("");
