...
class GreenSmartRebuildPanel extends HTMLElement {
  ...
  renderR7SettingsSystemActionModal() {
    ...
    if (modal.kind === "center-list") {
      ...
      const rowHtml = rows.map((row) => this.renderR7CdaCompactListRow({ selected: row === selected, attrs: `data-r7-settings-system-center-row="${row.id}" data-r7-settings-system-center-list-item-button="${row.id}" data-r7-settings-system-center-row-selected="${row === selected ? 'true' : 'false'}"`, columns: [`<b>${row.name}</b>`, `<span>${row.baseUrl}</span>`, `<span>${row.connectionStatus}</span>`, `<span>${row.credentialState === "configured" ? "[REDACTED]" : "missing"}</span>`] })).join("");
      const listPanel = this.renderR7CdaListPanel({ title: "Center 목록", columns: ["이름", "URL", "상태", "인증"], rowsHtml: rowHtml, footer: `<span>총 ${rows.length}건</span>`, attrs: 'data-r7-settings-system-center-list-panel' });
    ...
    if (modal.kind === "errors") {
      ...
      const listPanel = this.renderR7CdaListPanel({ title: "DB/API 오류 목록", columns: ["범위", "상태", "건수", "힌트"], rowsHtml: rowHtml, footer: `<span>총 ${rows.length}건</span><button type="button" data-r7-settings-system-errors-action="refresh-watchdog" style="border:1px solid #badcc8;border-radius:8px;background:#f0fbf4;color:#25804a;padding:6px 10px;font-weight:950;">watchdog 재검사</button>`, attrs: 'data-r7-settings-system-errors-list-panel' });
  ...
  renderR7SettingsDeviceSensorMappingSubtab(zones = []) {
    ...
    const errorCard = this.renderR7CdbSummaryCard({ key: "error-basic", icon: "mdi:alert-circle-check-outline", title: "오류 기본 정보", primary: `<span data-r7-settings-device-error-primary>${inactiveMappings ? `${inactiveMappings}건 점검` : "오류 없음"}</span>`, rows: [this._r7SettingsGreenhouseValueRow("점검", `${inactiveMappings}건`, 'data-r7-settings-device-error-row="needs-check"'), this._r7SettingsGreenhouseValueRow("미연결", `${visibleMappings.length ? 0 : 1}건`, 'data-r7-settings-device-error-row="unlinked"'), this._r7SettingsGreenhouseValueRow("연결 상태", inactiveMappings || !visibleMappings.length ? "확인 필요" : "정상")], tone: inactiveMappings || !visibleMappings.length ? "amber" : "green", statusKey: inactiveMappings || !visibleMappings.length ? "needs-verification" : "normal-ready", extraAttrs: 'data-r7-settings-device-card="error-basic" data-r7-settings-device-error-common-card="approval-needed" data-r7-settings-device-process="group-device-link" data-r7-settings-device-group-link-stage="device-to-group"' });
    ...
    const actions = [
      actionCard({ kind: "device-create", title: "장치 추가", icon: "mdi:devices", primary: "먼저 장치를 등록", note: "환기창·팬·밸브 등 실행 대상 장치를 먼저 추가합니다.", addLabel: "장치 추가", addAttr: 'data-r7-settings-device-create-button data-r7-settings-device-process="device-add-first"', shortcutLabel: "장치 목록", shortcutAttr: 'data-r7-settings-device-list-shortcut' }),
      actionCard({ kind: "device-link", title: "연결", icon: "mdi:link-variant", primary: "장치와 센서 연결", note: "등록된 장치와 센서 entity를 구역 기준으로 연결합니다.", addLabel: "연결", firstIcon: "mdi:link-plus", addAttr: 'data-r7-settings-device-sensor-mapping-button data-r7-settings-device-process="group-device-link" data-r7-settings-device-group-link-stage="device-to-group"', shortcutLabel: "장비 구성", shortcutAttr: 'data-r7-settings-equipment-info-shortcut-button' }),
