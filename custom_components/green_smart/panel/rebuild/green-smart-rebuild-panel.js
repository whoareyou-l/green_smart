...
class GreenSmartRebuildPanel extends HTMLElement {
  ...
  renderR7SettingsPermissionMatrixModal() {
    ...
    const buckets = [{ bucket: "조회", steps: "기본 조회 / 상세 조회", admin: "allowed", owner: "allowed", staff: "allowed" }, { bucket: "기록", steps: "기록 작성 / 기록 수정", admin: "allowed", owner: "allowed", staff: "allowed" }, { bucket: "전략", steps: "전략 검토 / 전략 승인", admin: "allowed", owner: "allowed", staff: "readonly" }, { bucket: "실행", steps: "실행 요청 / 실행 허락", admin: "allowed", owner: "allowed", staff: "request" }, { bucket: "안전", steps: "안전 확인 / 인터록 해제 검토", admin: "allowed", owner: "review", staff: "readonly" }, { bucket: "고급설정", steps: "구역/작기 설정 / 권한 설정", admin: "allowed", owner: "review", staff: "none" }];
    const roles = [{ id: "admin", label: "admin", title: "관리자", summary: "전체 권한 · 시스템 설정", tone: "blue", stateKey: "admin" }, { id: "farm_owner", label: "farm_owner", title: "농장 소유자", summary: "운영 승인 · 전략 검토", tone: "green", stateKey: "owner" }, { id: "farm_staff", label: "farm_staff", title: "농장 작업자", summary: "기록 작성 · 조회 중심", tone: "amber", stateKey: "staff" }];
  ...
  renderR7SettingsGreenhouseZonesSubtab(zones) {
    ...
    const createCards = [
      ...
      createCard({ kind: "settings-equipment-mapping", title: "장치/센서 매핑", icon: "mdi:devices", primary: "매핑 확인 필요", note: "선택 구역의 센서와 장비를 연결합니다", addLabel: "장치/센서 매핑 열기", addAttr: 'data-r7-settings-device-sensor-mapping-button', shortcutLabel: "장비 구성", shortcutAttr: 'data-r7-settings-equipment-info-shortcut-button' }),
    ].join("");
  ...
  renderR7SettingsDeviceSensorMappingSubtab(zones = []) {
    ...
    const deviceCard = this.renderR7SettingsInfoCard({ key: "device-basic", icon: "mdi:devices", title: "장치 기본 정보", primary: `${totalDevices}개 장치`, rows: [this._r7SettingsGreenhouseValueRow("장치 유형", "환기/스크린 · 관수/양액"), this._r7SettingsGreenhouseValueRow("등록 수", `${totalDevices}개`), this._r7SettingsGreenhouseValueRow("제어 기준", "HA entity")], tone: "green", statusKey: "normal-ready", extraAttrs: 'data-r7-settings-device-card="device-basic" data-r7-settings-device-process="device-add-first"' });
    const groupCard = this.renderR7SettingsInfoCard({ key: "group-basic", icon: "mdi:view-grid-plus-outline", title: "그룹 기본 정보", primary: `${groupNames.length}개 그룹`, rows: [this._r7SettingsGreenhouseValueRow("그룹 수", `${groupNames.length}개`), this._r7SettingsGreenhouseValueRow("구역 FK", "필수"), this._r7SettingsGreenhouseValueRow("대상 구역", zoneNames)], tone: inactiveMappings ? "amber" : "green", statusKey: inactiveMappings ? "needs-verification" : "normal-ready", extraAttrs: 'data-r7-settings-device-card="group-basic" data-r7-settings-device-process="group-create-zone-fk" data-r7-settings-device-group-zone-fk="required"' });
    const errorRows = this.renderR7CommonCardDataRows([
      { label: "점검", meta: `${inactiveMappings}건`, icon: "mdi:alert-circle-outline", tone: inactiveMappings ? "amber" : "green", extraAttrs: 'data-r7-settings-device-error-row="needs-check"' },
      { label: "미연결", meta: `${visibleMappings.length ? 0 : 1}건`, icon: "mdi:link-off", tone: visibleMappings.length ? "green" : "amber", extraAttrs: 'data-r7-settings-device-error-row="unlinked"' },
    ], { rowKind: "settings-device-error" });
    const errorCard = this.renderR7CommonCardShell({ kind: "settings-device-error-basic", section: "settings-device-error-basic", icon: "mdi:alert-circle-check-outline", title: "오류 기본 정보", statusKey: inactiveMappings || !visibleMappings.length ? "needs-verification" : "normal-ready", tone: "amber", primary: `<span data-r7-settings-device-error-primary>${inactiveMappings ? `${inactiveMappings}건 점검` : "오류 없음"}</span>`, note: inactiveMappings || !visibleMappings.length ? "장치/그룹 연결 상태를 확인하세요." : "장치/그룹 연결 오류가 없습니다.", extraAttrs: 'data-r7-settings-device-card="error-basic" data-r7-settings-device-error-common-card="approval-needed" data-r7-settings-device-process="group-device-link" data-r7-settings-device-group-link-stage="device-to-group"', html: errorRows });
    ...
    const actions = [
      actionCard({ kind: "device-add", title: "장치 추가", icon: "mdi:devices", primary: "먼저 장치를 등록", note: "환기창·팬·밸브 등 실행 대상 장치를 먼저 추가합니다.", addLabel: "장치 추가", addAttr: 'data-r7-settings-device-create-button data-r7-settings-device-process="device-add-first"', shortcutLabel: "장치 목록", shortcutAttr: 'data-r7-settings-device-list-shortcut' }),
      actionCard({ kind: "group-add", title: "그룹 추가", icon: "mdi:view-grid-plus-outline", primary: "구역 FK 필수", note: "그룹은 구역 FK 기준으로 관리합니다.", addLabel: "그룹 추가", addAttr: 'data-r7-settings-device-group-create-button data-r7-settings-device-process="group-create-zone-fk" data-r7-settings-device-group-zone-fk="required"', shortcutLabel: "그룹-장치 연결", shortcutAttr: 'data-r7-settings-device-group-link-button data-r7-settings-device-process="group-device-link" data-r7-settings-device-group-link-stage="device-to-group"' }),
    ].join("");
    ...
    const deviceList = this.renderR7CommonRecentPanel({ kind: "settings-device-list", title: "장치 목록", icon: "mdi:format-list-bulleted", statusKey: inactiveMappings ? "needs-verification" : "normal-ready", tone: inactiveMappings ? "amber" : "green", rows: listRows, limit: Number.POSITIVE_INFINITY, rowKind: "settings-device-list", extraAttrs: 'data-r7-settings-device-list-panel data-r7-settings-device-table-header data-r7-settings-device-group-link-stage="device-to-group"' });
    return `<section data-r7-settings-device-sensor-mapping data-r7-settings-device-mapping-layout="device-group-error-device-list" style="display:grid;gap:12px;"><div data-r7-settings-device-summary-grid style="display:grid;grid-template-columns:repeat(3,minmax(210px,1fr));gap:12px;">${deviceCard}${groupCard}${errorCard}</div><div data-r7-settings-device-action-row style="display:grid;grid-template-columns:repeat(3,minmax(210px,1fr));gap:12px;">${actions}</div>${deviceList}</section>`;
  }
            ...
            ? `${(() => {
              ...
              return `<section data-r7-settings-users-permissions data-r7-settings-users-data-source="${source}" data-r7-settings-users-permissions-image-layout="true" data-r7-settings-users-record-card-layout="true" data-r7-settings-users-layout-order="approval-audit-matrix-user-list" data-r7-settings-users-typography="aligned-compact" data-r7-settings-users-grid-align="centered" style="display:grid;grid-template-columns:repeat(3,minmax(220px,1fr));gap:12px;align-items:stretch;line-height:1.35;">
                ...
                ${this.renderR7CommonCardShell({
                  kind: "settings-permission-matrix-summary", section: "settings-permission-matrix-summary", icon: "mdi:table-key", title: "역활별 권한", subtitle: '<span data-r7-settings-role-permission-count-note>총 3개 역할</span>', statusKey: "normal-ready", tone: "blue", extraAttrs: 'data-r7-settings-users-card="permission-matrix" data-r7-settings-permission-matrix-detailed="true" data-r7-common-data-limit="3"', html: this.renderR7CommonCardDataRows([{ label: "admin", meta: "전체 권한 · 시스템 설정", icon: "mdi:shield-crown-outline", tone: "blue", extraAttrs: 'data-r7-settings-role-permission-summary-row="admin"' }, { label: "farm_owner", meta: "운영 승인 · 전략 검토", icon: "mdi:account-tie-outline", tone: "green", extraAttrs: 'data-r7-settings-role-permission-summary-row="farm_owner"' }, { label: "farm_staff", meta: "기록 작성 · 조회 중심", icon: "mdi:account-outline", tone: "amber", extraAttrs: 'data-r7-settings-role-permission-summary-row="farm_staff"' }], { rowKind: "settings-role-permission-summary" }), actions: [this.renderR7CommonCardButton({ label: "전체 역활별 권한 보기", icon: "mdi:table-eye", tone: "blue", extraAttrs: 'data-r7-settings-permission-matrix-button data-r7-settings-modal-skip-record-binding="true"' })]
                })}
