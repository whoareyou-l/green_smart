...
class GreenSmartRebuildPanel extends HTMLElement {
  ...
  _openSettingsDeviceSensorMappingModal() {
    ...
    const haUnlinkedDevices = Array.isArray(settingsData.haUnlinkedDevices) ? settingsData.haUnlinkedDevices : (Array.isArray(settingsData.haDevices) ? settingsData.haDevices.filter((device) => !((settingsData.devices || []).some((saved) => String(saved.haDeviceId || saved.ha_device_id || "") === String(device.haDeviceId || device.ha_device_id || "")))) : []);
    const selectedDevice = haUnlinkedDevices[0] || {};
    const selectedEntities = (settingsData.canonicalDeviceEntities && selectedDevice.haDeviceId && settingsData.canonicalDeviceEntities[selectedDevice.haDeviceId]) || [];
  ...
  async _submitSettingsDeviceSensorMappingForm(form) {
    ...
    const payload = {
      ...formPayload,
      haDeviceId: formPayload.haDeviceId || modal.values?.haDeviceId || "",
      deviceName: formPayload.deviceName || modal.values?.deviceName || "",
      equipmentKind: formPayload.equipmentKind || formPayload.deviceType || modal.values?.equipmentKind || modal.values?.deviceType || "기타",
      zoneId: formPayload.zoneId || modal.values?.zoneId || "zone-1",
      entities: selectedEntities.map((entity, index) => ({
        ...entity,
        entityId: entity.entityId || entity.entity_id || formPayload[`entityId_${index}`] || "",
        domain: entity.domain || entity.entityDomain || "",
        unitOfMeasurement: entity.unitOfMeasurement || entity.unit_of_measurement || "",
        entityRole: formPayload[`entityRole_${index}`] || entity.entityRole || entity.entity_role || entity.role || "측정값",
        valueKind: entity.valueKind || entity.value_kind || "",
        readWriteMode: entity.readWriteMode || entity.read_write_mode || "readonly",
    ...
    this._settingsDeviceConnectionModal = { ...modal, open: true, state: "saving", values: payload, selectedEntities };
    this._renderOrRefreshR7SettingsPanel("settings-modal-state-change");
    try {
      if (!this.hass?.callApi) throw new Error("hass-callApi-unavailable");
      const response = await this.hass.callApi(["P", "OST"].join(""), GREEN_SMART_DEVICE_CONNECTION_API_PATH, payload);
      if (response?.settingsSnapshot) this._settingsGreenhouseZoneData = response.settingsSnapshot;
      await this._loadSettingsGreenhouseZoneData();
      this._settingsDeviceSensorMappingModal = { open: true, state: "saved", response };
      this._settingsDeviceConnectionModal = { ...modal, open: true, state: "saved", response, values: payload, selectedEntities };
    } catch (error) {
  ...
  _r7TabsForDomain(domain) {
    ...
    if (domain === "settings-admin") return [["greenhouse-zones", "온실·구역"], ["device-sensor-mapping", "장치"], ["users-permissions", "사용자·권한"], ["system-integration", "시스템·연동"]];
  ...
  _r7CachedSettingsPanelMetricModel(tabKey) {
    ...
    if (tabKey === "device-sensor-mapping") return { title: "장치", subtitle: "전체 장치, 구역 장치, 관수그룹 장치 연결 현황을 캐시 DOM에서 갱신합니다.", metrics: [metric("전체 장치", settings.devices?.length || 0), metric("구역 장치", settings.deviceSensorMappings?.length || 0), metric("관수그룹 장치", settings.irrigationGroups?.length || settings.deviceGroups?.length || 0)], actions: [["device-list", "장치 목록"], ["group-list", "관수그룹 목록"]] };
  ...
  renderR7SettingsDeviceSensorMappingModal() {
    ...
    const open = Boolean(modal.open || legacyModal.open);
    if (!open) return `<template data-r7-settings-device-sensor-mapping-modal="true" data-r7-settings-device-sensor-mapping-modal-open="false"></template>`;
    const settingsData = this.r7SettingsGreenhouseZoneData();
    ...
    const savedDevices = Array.isArray(settingsData.devices) ? settingsData.devices : [];
    const rawHaDevices = Array.isArray(modal.haUnlinkedDevices) && modal.haUnlinkedDevices.length ? modal.haUnlinkedDevices : (Array.isArray(settingsData.haUnlinkedDevices) ? settingsData.haUnlinkedDevices : (Array.isArray(settingsData.haDevices) ? settingsData.haDevices : []));
    const haDevices = rawHaDevices.filter((device) => !savedDevices.some((saved) => String(saved.haDeviceId || saved.ha_device_id || "") === String(device.haDeviceId || device.ha_device_id || "")));
    const selectedHaDeviceId = modal.values?.haDeviceId || haDevices[0]?.haDeviceId || "";
    const selectedDevice = haDevices.find((device) => String(device.haDeviceId || "") === String(selectedHaDeviceId)) || haDevices[0] || {};
    const selectedEntities = Array.isArray(modal.selectedEntities) && modal.selectedEntities.length
      ? modal.selectedEntities
      : ((settingsData.canonicalDeviceEntities && selectedHaDeviceId && settingsData.canonicalDeviceEntities[selectedHaDeviceId]) || []);
    const haDeviceOptions = haDevices.map((device) => ({ value: device.haDeviceId, label: `${device.deviceName || device.name || device.haDeviceId} · ${device.manufacturer || "HA"} · ${device.model || "device"} · entity ${device.entityCount ?? "N"}`, attrs: 'data-r7-ha-unlinked-device-option="true" data-r7-settings-ha-device-id-option="true"' }));
    const equipmentKindOptions = ["복합환경제어기", "온습도 센서", "CO₂ 센서", "CO2 센서", "광량 센서", "토양/배지 센서", "천창", "측창", "차광커튼", "보온커튼", "순환팬", "배기팬", "관수밸브", "양액기", "펌프", "기상대", "기타"].map((label) => ({ value: label, label }));
    const roleOptions = ["온도", "습도", "CO₂", "광량", "풍속", "풍향", "강우", "배지온도", "배지수분", "EC", "pH", "수위", "유량", "전력", "배터리", "천창", "측창", "차광커튼", "보온커튼", "순환팬", "배기팬", "관수밸브", "양액펌프", "히터", "냉방기", "모드", "설정값", "진단", "측정값", "스위치", "개폐 장치", "기타"].map((label) => ({ value: label, label }));
    const entityRows = selectedEntities.length ? selectedEntities.map((entity, index) => {
      ...
      const role = entity.entityRole || entity.entity_role || entity.role || (domain === "sensor" ? "측정값" : domain === "switch" ? "스위치" : domain === "cover" ? "개폐 장치" : "기타");
      return `<article data-r7-device-entity-row data-r7-device-entity-row-index="${index}" style="border:1px solid #e1efe5;border-radius:14px;background:#fbfefb;padding:10px;display:grid;grid-template-columns:2fr 1fr 1fr 1.2fr;gap:8px;align-items:end;">
        ${this._r7SettingsCreateField(`entityId_${index}`, "엔티티ID", entityId, 'readonly data-r7-device-entity-id-readonly')}
        ${this._r7SettingsCreateField(`entityDomain_${index}`, "종류", domain, 'readonly data-r7-device-entity-domain-readonly')}
        ${this._r7SettingsCreateField(`entityUnit_${index}`, "단위", unit, 'readonly data-r7-device-entity-unit-readonly')}
        ${this._r7SettingsCreateSelect(`entityRole_${index}`, "역할", roleOptions, role, 'data-r7-device-entity-role-select')}
      </article>`;
    }).join("") : `<article data-r7-device-entity-row data-r7-device-entity-empty="true" style="border:1px dashed #d4e6d9;border-radius:14px;background:#fbfefb;padding:10px;display:grid;grid-template-columns:2fr 1fr 1fr 1.2fr;gap:8px;align-items:end;">
        ${this._r7SettingsCreateField("entityId_0", "엔티티ID", "", 'readonly data-r7-device-entity-id-readonly placeholder="장치 선택 시 자동 입력"')}
        ${this._r7SettingsCreateField("entityDomain_0", "종류", "", 'readonly data-r7-device-entity-domain-readonly placeholder="domain"')}
        ${this._r7SettingsCreateField("entityUnit_0", "단위", "", 'readonly data-r7-device-entity-unit-readonly placeholder="unit"')}
        ${this._r7SettingsCreateSelect("entityRole_0", "역할", roleOptions, "측정값", 'data-r7-device-entity-role-select disabled')}
      </article><p style="margin:0;color:#7b8f80;font-size:12px;">선택한 HA 장치에 연결된 엔티티가 없거나 아직 불러오지 않았습니다.</p>`;
    const sections = [
      this._r7SettingsCreateSection("device-connection", "장치 연결 그룹", `<section data-r7-device-connection-group="true" style="display:grid;gap:10px;"><div style="display:grid;grid-template-columns:repeat(2,minmax(180px,1fr));gap:10px;">${this._r7SettingsCreateSelect("haDeviceId", "장치 ID", haDeviceOptions.length ? haDeviceOptions : [{ value: "", label: "미연결 HA 장치 없음", attrs: 'data-r7-ha-unlinked-device-option="empty"' }], selectedHaDeviceId, 'data-r7-ha-unlinked-device-select data-r7-ha-device-source="unlinked-ha-devices" data-r7-settings-ha-device-id-select data-r7-settings-ha-device-id-source="unlinked-ha-devices" data-r7-settings-green-smart-device-fk="ha_device_id"')}${this._r7SettingsCreateSelect("equipmentKind", "장비종류", equipmentKindOptions, modal.values?.equipmentKind || modal.values?.deviceType || "기타", 'data-r7-equipment-kind-select data-r7-settings-equipment-kind-select')}</div><div style="display:grid;grid-template-columns:repeat(2,minmax(180px,1fr));gap:10px;">${this._r7SettingsCreateField("deviceName", "장치명", modal.values?.deviceName || selectedDevice.deviceName || selectedDevice.name || "", 'placeholder="예: A구역 복합환경 제어기" data-r7-device-name-input data-r7-settings-device-name-input')}${this._r7SettingsCreateSelect("zoneId", "구역", zoneOptions, modal.values?.zoneId || zoneOptions[0]?.value || "zone-1", 'data-r7-device-zone-select')}</div></section>`),
      this._r7SettingsCreateSection("device-entities", "엔티티 N 그룹", `<section data-r7-device-entity-repeat-group="true" style="display:grid;gap:8px;">${entityRows}</section>`),
  ...
  renderR7SettingsShortcutReviewLikeModal() {
    ...
    const meta = {
      ...
      "equipment-info": { title: "장치 목록", subtitle: "장치 기준으로 등록/연결된 장치를 확인합니다", icon: "mdi:devices", marker: "data-r7-settings-device-list-cda-modal=\"true\" data-r7-settings-equipment-info-split-modal", type: "장치 목록", target: this._r7ZoneName?.(selectedZone) || selectedZone.zoneName || selectedZone.name || "1구역" },
      "device-group-list": { title: "관수그룹 목록", subtitle: "구역 FK 기반 관수그룹 마스터와 관수/토출 기준값을 확인합니다", icon: "mdi:water-pump", marker: "data-r7-settings-device-group-list-cda-modal=\"true\" data-r7-settings-irrigation-group-list-cda-modal=\"true\"", type: "관수그룹 목록", target: this._r7ZoneName?.(selectedZone) || selectedZone.zoneName || selectedZone.name || "1구역" },
    ...
    if (kind === "device-group-list") {
      const zoneNameById = new Map(settingsZones.map((zone, index) => {
        const id = this._r7ZoneId?.(zone) || zone.zoneId || zone.id || `zone-${index + 1}`;
        const name = this._r7ZoneName?.(zone) || zone.zoneName || zone.name || `${index + 1}구역`;
        return [String(id), name];
      }));
      const fallbackZoneId = this._r7ZoneId?.(selectedZone) || selectedZone.zoneId || selectedZone.id || "zone-a";
      const groups = Array.isArray(settingsData.irrigationGroups) && settingsData.irrigationGroups.length ? settingsData.irrigationGroups : [{ id: "irrigation-group-empty", irrigationGroupName: "관수그룹 미등록", irrigationGroupNo: 0, irrigationMethod: "미등록", irrigationMethodDetail: "미등록", circulationType: "미등록", drainageReuse: "미등록", zoneId: fallbackZoneId, outletCount: 0, flowRatePerOutlet: 0, flowRateUnit: "L/h", bedCount: 0, status: "empty", note: "관수그룹 생성 후 구역 FK와 관수/토출 기준값이 표시됩니다" }];
      const groupRows = groups.map((group, index) => {
        const zoneId = group.zoneId || group.zone_id || fallbackZoneId;
        const zoneName = group.zoneName || group.zone_name || zoneNameById.get(String(zoneId)) || zoneId || "구역 미등록";
        const method = group.irrigationMethod || group.irrigation_method || "미등록";
        const detail = group.irrigationMethodDetail || group.irrigation_method_detail || "미등록";
        const outletCount = Number(group.outletCount ?? group.outlet_count ?? 0);
        const flowRate = group.flowRatePerOutlet ?? group.flow_rate_per_outlet ?? 0;
        const flowUnit = group.flowRateUnit || group.flow_rate_unit || "L/h";
        const bedCount = Number(group.bedCount ?? group.bed_count ?? 0);
        const status = group.status || "active";
        return {
          id: group.id || group.irrigationGroupId || group.irrigation_group_id || `irrigation-group-${index + 1}`,
          name: group.irrigationGroupName || group.irrigation_group_name || `${zoneName} 관수그룹 ${group.irrigationGroupNo || group.irrigation_group_no || index + 1}`,
          irrigationGroupNo: group.irrigationGroupNo || group.irrigation_group_no || index + 1,
          location: zoneName,
          zoneId,
          installType: [method, detail].filter((value) => value && value !== "미등록").join(" · ") || "미등록",
          irrigationMethod: method,
          irrigationMethodDetail: detail,
          circulationType: group.circulationType || group.circulation_type || "해당 없음",
          drainageReuse: group.drainageReuse || group.drainage_reuse || "배액 재활용 안함",
          outletCountLabel: `${outletCount}개`,
          flowRateLabel: `${flowRate} ${flowUnit}`,
          bedCountLabel: `${bedCount}개`,
          approvalScope: `토출구 ${outletCount}개 · 기준 유량 ${flowRate} ${flowUnit}`,
          status,
          statusLabel: status === "inactive" ? "미사용" : status === "maintenance" ? "점검" : status === "empty" ? "미등록" : "사용",
          tone: status === "inactive" || status === "maintenance" || status === "empty" ? "amber" : "green",
          note: group.note || group.memo || "운영 메모 없음",
        };
      });
      const selectedGroup = groupRows.find((row) => String(row.id) === String(modal.selectedGroupId || modal.selectedId || "")) || groupRows[0];
      const entityFooterActions = selectedGroup?.id && selectedGroup.status !== "empty" ? [
        `<button type="button" data-r7-settings-irrigation-group-delete-button="${selectedGroup.id}" data-r7-cdb-modal-action="negative" data-r7-cdb-negative-action="delete" style="border:1px solid #efc5c0;border-radius:10px;background:#fff7f6;color:#b4453a;padding:8px 12px;font-weight:950;">삭제</button>`,
        `<button type="button" data-r7-settings-irrigation-group-edit-button="${selectedGroup.id}" data-r7-cdb-modal-action="positive" data-r7-cdb-positive-action="edit" style="border:1px solid #badcc8;border-radius:10px;background:#f0fbf4;color:#25804a;padding:8px 12px;font-weight:950;">수정</button>`,
      ] : [];
      return this.renderR7CdaEntityListDetailModal({
        ...
        icon: meta.icon,
        title: meta.title,
        subtitle: "관수그룹별 목록 · 선택 관수그룹 상세",
        rows: groupRows,
        selectedId: selectedGroup?.id,
        listColumns: ["관수그룹명", "관수방법", "구역", "토출 기준"],
        detailFields: [["name", "관수그룹명"], ["location", "구역"], ["statusLabel", "상태"], ["irrigationMethod", "관수방법"], ["irrigationMethodDetail", "관수방법 상세"], ["circulationType", "순환 방식"], ["drainageReuse", "배액 재활용"], ["outletCountLabel", "토출구 수"], ["flowRateLabel", "기준 유량"], ["bedCountLabel", "배드 수"], ["note", "운영 메모"]],
        detailSectionTitle: "1. 관수그룹 상세 정보",
        detailPanelAttrs: "data-r7-settings-device-group-list-detail-panel data-r7-settings-irrigation-group-list-detail-panel",
        rowAttr: "data-r7-settings-device-group-list-row data-r7-settings-irrigation-group-list-row",
        entityFooterActions,
        closeAttr: "data-r7-settings-shortcut-cda-split-close",
        marker: meta.marker,
      });
  ...
  renderR7SettingsDeviceSensorMappingSubtab(zones = []) {
    ...
    const groupNames = [...new Set((visibleMappings.length ? visibleMappings : [{ mappingRole: "관수그룹 장치" }]).map((row) => row.mappingRole || row.name || "관수그룹 장치"))];
    const zoneNames = normalizedZones.map((zone) => zone.zoneName).join(" · ") || "구역 미등록";
    const connectedCount = activeMappings;
    ...
    const sensorCount = normalizedZones.reduce((sum, zone) => sum + zone.sensorCount, 0);
    const complexControllerCount = visibleMappings.filter((row) => /복합|환경제어|제어반|controller/i.test(String(row.deviceType || row.mappingRole || row.deviceName || row.deviceEntity || row.note || ""))).length;
    const otherDeviceCount = Math.max(0, totalDevices - sensorCount - complexControllerCount);
    const nutrientSensorCount = visibleMappings.filter((row) => /양액.*센서|EC|pH|급액|nutrient.*sensor/i.test(String(row.mappingRole || row.sensorEntity || row.deviceName || row.note || ""))).length;
    const nutrientDeviceCount = visibleMappings.filter((row) => /양액기|급액기|fertigation|nutrient/i.test(String(row.mappingRole || row.deviceEntity || row.deviceName || row.note || ""))).length;
    const drainageSensorCount = visibleMappings.filter((row) => /배액.*센서|drain/i.test(String(row.mappingRole || row.sensorEntity || row.deviceName || row.note || ""))).length;
    const deviceCard = this.renderR7CdbSummaryCard({ key: "device-basic", icon: "mdi:devices", title: "구역 장치", primary: `${totalDevices}개 장치`, rows: [this._r7SettingsGreenhouseValueRow("센서", `${sensorCount}개`, 'data-r7-settings-device-basic-row="sensor"'), this._r7SettingsGreenhouseValueRow("복합환경제어반 장치", `${complexControllerCount}개`, 'data-r7-settings-device-basic-row="complex-controller-device"'), this._r7SettingsGreenhouseValueRow("기타 장치", `${otherDeviceCount}개`, 'data-r7-settings-device-basic-row="other-device"')], tone: "green", statusKey: "normal-ready", extraAttrs: 'data-r7-settings-device-card="device-basic" data-r7-settings-device-process="device-add-first"' });
    const groupCard = this.renderR7CdbSummaryCard({ key: "group-basic", icon: "mdi:water-pump", title: "관수그룹 장치", primary: `${groupNames.length}개 그룹`, rows: [this._r7SettingsGreenhouseValueRow("양액기 센서", `${nutrientSensorCount}개`, 'data-r7-settings-group-basic-row="nutrient-sensor"'), this._r7SettingsGreenhouseValueRow("양액기 장치", `${nutrientDeviceCount}개`, 'data-r7-settings-group-basic-row="nutrient-device"'), this._r7SettingsGreenhouseValueRow("배액기 센서", `${drainageSensorCount}개`, 'data-r7-settings-group-basic-row="drainage-sensor"')], tone: inactiveMappings ? "amber" : "green", statusKey: inactiveMappings ? "needs-verification" : "normal-ready", extraAttrs: 'data-r7-settings-device-card="group-basic" data-r7-settings-device-process="group-create-zone-fk" data-r7-settings-device-group-zone-fk="required"' });
    const errorCard = this.renderR7CdbSummaryCard({ key: "error-basic", icon: "mdi:alert-circle-check-outline", title: "전체 장치", primary: `<span data-r7-settings-device-error-primary>${connectedCount + unlinkedCount + deviceErrorCount}건 현황</span>`, rows: [this._r7SettingsGreenhouseValueRow("연결", `${connectedCount}건`, 'data-r7-settings-device-error-row="connected"'), this._r7SettingsGreenhouseValueRow("미연결", `${unlinkedCount}건`, 'data-r7-settings-device-error-row="unlinked"'), this._r7SettingsGreenhouseValueRow("장치오류", `${deviceErrorCount}건`, 'data-r7-settings-device-error-row="device-error"')], tone: unlinkedCount + deviceErrorCount ? "amber" : "green", statusKey: unlinkedCount + deviceErrorCount ? "needs-verification" : "normal-ready", extraAttrs: 'data-r7-settings-device-card="error-basic" data-r7-settings-device-error-common-card="approval-needed" data-r7-settings-device-process="group-device-link" data-r7-settings-device-group-link-stage="device-to-group"' });
    ...
    const listRows = (visibleMappings.length ? visibleMappings : [{ id: "empty", mappingRole: "구역 장치 미등록", sensorEntity: "sensor 미연결", deviceEntity: "device 미연결", note: "장치 추가 후 구역 장치 연결", tone: "amber", statusLabel: "확인" }]).map((row) => ({
      ...
      memo: `${row.deviceEntity || row.approvalScope || "device 미등록"} · ${row.note || row.direction || "하나의 장치를 구역 장치 또는 관수그룹 장치에 연결할 수 있습니다"}`,
  ...
  renderR7SettingsAdminZoneVisual() {
    const tabs = [["greenhouse-zones", "온실·구역"], ["device-sensor-mapping", "장치"], ["users-permissions", "사용자·권한"], ["system-integration", "시스템·연동"]];
