      ...
      : tabKey === "crop-cycle-objects"
        ...
        : tabKey === "device-sensor-mapping"
          ...
          : tabKey === "users-permissions"
            ? `<section data-r7-settings-users-permissions data-r7-settings-users-permissions-image-layout="true" data-r7-settings-users-typography="aligned-compact" data-r7-settings-users-grid-align="centered" style="display:grid;grid-template-columns:minmax(360px,1.05fr) minmax(420px,1.25fr);gap:12px;align-items:start;line-height:1.35;">
                <article data-r7-settings-users-card="user-list" style="border:1px solid #dcebe0;border-radius:18px;background:#fff;padding:14px;display:grid;gap:12px;box-shadow:0 8px 22px rgba(31,67,45,.04);">
                  <strong style="font-size:15px;color:#24323f;line-height:1.35;">사용자 목록</strong>
                  ...
                  <section data-r7-settings-users-card="approval-request" style="display:grid;gap:8px;border-top:1px solid #edf4ef;padding-top:10px;"><strong style="font-size:13px;color:#24323f;">사용자 승인 요청</strong><div style="display:grid;grid-template-columns:1fr .9fr .7fr .9fr;gap:8px;align-items:center;text-align:center;color:#52675b;font-size:11px;line-height:1.35;font-weight:900;"><span>요청자</span><span>요청 역할</span><span>요청 상태</span><span>승인 요청 허락</span></div>${[["staff02","farm_staff","대기"],["owner02","farm_owner","검토" ]].map(([name,role,state]) => `<div data-r7-settings-user-approval-request-row="${name}" style="display:grid;grid-template-columns:1fr .9fr .7fr .9fr;gap:8px;align-items:center;text-align:center;font-size:12px;line-height:1.35;color:#24323f;"><span style="font-weight:900;">${name}</span><span style="justify-self:center;border:1px solid #dcebe0;border-radius:999px;background:#f4fbf5;color:#31523b;padding:3px 7px;font-size:11px;font-weight:900;">${role}</span><span style="justify-self:center;border-radius:999px;background:#fff4d6;color:#9a6b10;padding:3px 7px;font-size:11px;font-weight:900;">${state}</span><span style="display:flex;justify-content:center;gap:6px;"><button type="button" data-r7-settings-users-action="approve-request" style="border:1px solid #badcc8;border-radius:8px;background:#fff;color:#31523b;padding:6px 8px;font-size:12px;line-height:1.35;font-weight:950;">허락</button><button type="button" data-r7-settings-users-action="reject-request" style="border:1px solid #ead4a2;border-radius:8px;background:#fff;color:#9a6b10;padding:6px 8px;font-size:12px;line-height:1.35;font-weight:950;">반려</button></span></div>`).join("")}</section>
                </article>
                <article data-r7-settings-users-card="permission-matrix" data-r7-settings-permission-matrix-detailed="true" style="border:1px solid #dcebe0;border-radius:18px;background:#fff;padding:14px;display:grid;gap:9px;box-shadow:0 8px 22px rgba(31,67,45,.04);">
                  <strong style="font-size:15px;color:#24323f;line-height:1.35;">권한 버킷 매트릭스</strong>
                  <div style="display:grid;grid-template-columns:.7fr 1.15fr .82fr .82fr .82fr .55fr;border:1px solid #edf4ef;border-radius:12px;overflow:hidden;font-size:12px;line-height:1.35;text-align:center;color:#24323f;">
                    ${[["조회","기본 조회 / 상세 조회","✅ 허용","✅ 허용","✅ 허용"],["기록","기록 작성 / 기록 수정","✅ 허용","✅ 허용","✅ 허용"],["전략","전략 검토 / 전략 승인","✅ 허용","✅ 허용","👁️ 읽기 전용"],["실행","실행 요청 / 실행 허락","✅ 허용","✅ 허용","🕘 요청 후 실행"],["안전","안전 확인 / 인터록 해제 검토","✅ 허용","🛡️ 확인","👁️ 읽기 전용"],["고급설정","구역/작기 설정 / 권한 설정","✅ 허용","🛡️ 확인","🔒 없음"]].map(([bucket,steps,admin,owner,staff], rowIndex) => [[bucket, `data-r7-settings-permission-bucket="${bucket}" data-r7-settings-permission-step-row="${bucket}"`],[steps, ""],[admin, rowIndex === 0 ? 'data-r7-settings-permission-role="admin"' : ""],[owner, rowIndex === 0 ? 'data-r7-settings-permission-role="farm_owner"' : ""],[staff, rowIndex === 0 ? 'data-r7-settings-permission-role="farm_staff"' : ""],[`<button type="button" data-r7-settings-permission-edit="${bucket}" style="border:1px solid #badcc8;border-radius:8px;background:#fff;color:#31523b;padding:5px 8px;font-size:12px;line-height:1.35;font-weight:950;">수정</button>`, ""]].map(([cell, attrs], colIndex) => `<span ${attrs} style="display:grid;align-items:center;justify-items:center;border-right:${colIndex === 5 ? '0' : '1px solid #edf4ef'};border-bottom:${rowIndex === 5 ? '0' : '1px solid #edf4ef'};padding:8px 8px;min-height:40px;font-weight:${colIndex <= 1 ? '950' : '800'};background:${colIndex <= 1 ? '#fbfdfb' : '#fff'};color:${String(cell).includes('없음') ? '#5d6871' : String(cell).includes('확인') || String(cell).includes('요청') ? '#9a6b10' : String(cell).includes('읽기') ? '#326aa5' : '#31523b'};">${cell}</span>`).join("")).join("")}
                  </div>
                  <small style="color:#78927f;font-size:11px;line-height:1.35;">ⓘ 권한 단계는 표시용이며 실제 허락/차단은 backend enforcement와 승인 요청 상태를 따릅니다.</small><span data-r7-settings-permission-bucket-card style="display:none;">조회 · 기록 · 전략 · 실행 · 안전 · 고급설정</span>
                </article>
                <article data-r7-settings-users-card="approval-queue" style="border:1px solid #dcebe0;border-radius:18px;background:#fff;padding:14px;display:grid;gap:10px;">
                  <strong style="font-size:15px;color:#24323f;line-height:1.35;">승인 필요 작업</strong>
                  ${[["자동제어 활성화","farm_owner 허락"],["안전 리밋 변경","farm_owner 허락"],["HA entity mapping 변경","admin 허락"]].map(([label, approver]) => `<div data-r7-settings-approval-row="${label}" style="display:flex;align-items:center;justify-content:space-between;gap:10px;font-size:12px;line-height:1.35;color:#24323f;padding:4px 0;"><span>⌛ ${label}</span><span style="border:1px solid #dcebe0;border-radius:999px;background:#fbfdfb;color:#52675b;padding:4px 8px;font-size:11px;font-weight:900;">${approver}</span></div>`).join("")}
                  <button type="button" data-r7-settings-users-action="approval-all" style="border:1px solid #badcc8;border-radius:8px;background:#fff;color:#31523b;padding:9px 10px;font-size:12px;line-height:1.35;font-weight:950;">모든 승인 요청 확인 〉</button>
                </article>
                <article data-r7-settings-users-card="audit-log" style="border:1px solid #dcebe0;border-radius:18px;background:#fff;padding:14px;display:grid;gap:10px;">
                  <strong style="font-size:15px;color:#24323f;line-height:1.35;">감사 로그</strong>
                  ${[["A","admin","역할 허락: staff01 → farm_staff","2026-06-30 09:12"],["O","owner01","안전 정책 허락: 온도 고한 28→30℃","2026-06-30 08:45"],["S","staff01","생육조사 작성: A구역 · 상추","2026-06-30 08:10"]].map(([initial,user,summary,time]) => `<div data-r7-settings-audit-row="${user}" style="display:grid;grid-template-columns:auto 1fr auto;gap:8px;align-items:center;font-size:12px;line-height:1.35;color:#24323f;"><i style="width:20px;height:20px;border-radius:999px;background:#43ad5e;color:#fff;display:inline-grid;place-items:center;font-size:10px;font-style:normal;">${initial}</i><span><b>${user}</b><br><small style="color:#52675b;line-height:1.35;">${summary}</small></span><time style="color:#52675b;font-size:11px;">${time}</time></div>`).join("")}
                  <button type="button" data-r7-settings-users-action="audit-all" style="border:1px solid #badcc8;border-radius:8px;background:#fff;color:#31523b;padding:9px 10px;font-size:12px;line-height:1.35;font-weight:950;">전체 감사 로그 보기 ↗</button>
                </article>
              </section>`
