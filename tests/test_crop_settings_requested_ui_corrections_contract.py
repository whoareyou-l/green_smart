...
def test_v1978_version_markers():
    assert '"version": "1.9.86"' in _read(MANIFEST)
    assert 'const VERSION = "1.9.86"' in _read(PANEL)
    assert 'v1.9.86' in _read(PANEL)[:200]
    assert 'EDGE_VERSION = "1.9.86"' in _read(CENTRAL)
    assert '기준 버전: `v1.9.86`' in _read(UI_DOC)
