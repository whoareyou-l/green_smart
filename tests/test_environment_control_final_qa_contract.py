...
def test_v1998_environment_final_qa_versions_and_docs():
    panel = _read(PANEL)
    assert '"version": "1.9.98"' in _read(MANIFEST)
    assert 'const VERSION = "1.9.98"' in panel
    assert 'v1.9.98' in panel[:200]
    plan = _read(PLAN)
    docs = _read(UI_DOC) + "\n" + _read(MASTER)
    assert "Status: implemented in `v1.9.98`" in plan
    assert "v1.9.98 Environment Control final QA" in docs
...
def test_environment_final_qa_covers_all_seven_tabs():
    ...
    assert 'if (tab === "setpoints")' in panel
    assert 'if (tab === "rules")' in panel
    assert 'if (tab === "ai")' in panel
