...
def test_r7_067_visible_settings_tabs_are_only_current_seven_tabs():
    html = _render_settings()
    visible_nav = html.split('data-r7-domain-subtab-panel', 1)[0]
    expected = ['온실·구역', '작기·작물 객체', '장치·센서 매핑', '사용자·권한', '안전·승인 정책', '시스템·연동', '진단·감사']
    for label in expected:
        assert label in visible_nav
    assert visible_nav.count('data-r7-domain-subtab-key=') == 7
...
def test_r7_067_documented():
    doc = _read(DOC)
    for phrase in ('구버전 탭 버튼 노출 제거', '7개만 표시', 'hidden compatibility marker', '도메인 소유권', 'RBAC 정책'):
