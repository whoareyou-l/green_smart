...
def test_v1973_crop_visual_corrections_version_and_plan_shift():
    ...
    assert '"version": "1.9.79"' in manifest
    assert 'const VERSION = "1.9.79"' in panel
    assert 'v1.9.79' in panel[:200]
    assert 'EDGE_VERSION = "1.9.79"' in central
    assert '기준 버전: `v1.9.79`' in docs
