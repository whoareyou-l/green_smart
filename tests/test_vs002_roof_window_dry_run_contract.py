...
def test_vs002_version_surfaces_are_current():
    ...
    assert '"version": "1.14.40"' in manifest
    assert 'const VERSION = "1.14.40"' in panel
    assert "v1.14.40" in panel[:200]
    assert "기준 버전: `v1.14.40`" in readme
    assert "기준 버전: `v1.14.40`" in workflow
