...
def test_rs027_version_surfaces_are_aligned_to_1_12_26():
    ...
    assert 'const VERSION = "1.12.26"' in _read(LEGACY_PANEL)
    assert 'REBUILD_VERSION = "1.12.26"' in _read(REBUILD_PANEL)
    for path in (DOC, INTERFACE_SPEC, WORKFLOW_SPEC, FAILSAFE_SPEC, PRODUCT_PLAN, LEGACY_INVENTORY):
        assert "v1.12.26" in _read(path)
...
def test_docs_specs_plan_and_inventory_record_rs027_and_rs_series_completion():
    ...
    for marker in ("Phase R4.23 — Virtual rehearsal pass/fail review projection", "Status:** `v1.12.26`에서 Virtual rehearsal pass/fail review projection 완료", "No production route removal in RS-027", "No DB migration in RS-027", "No write/mutation in RS-027", "RS sequence complete before R5 scaffold"):
