...
def test_rs008_version_surfaces_are_v1127():
    ...
    assert 'const VERSION = "1.15.41"' in _read(PANEL)
    assert "v1.15.41" in _read(PLAN)
    assert "v1.15.41" in _read(PRODUCT_PLAN)
    assert "v1.15.41" in _read(MASTER)
    assert "v1.15.41" in _read(MASTER_DB)
