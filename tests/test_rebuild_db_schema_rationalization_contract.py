...
def test_r4_version_surfaces_are_v1114():
    assert '"version": "1.11.17"' in _read(MANIFEST)
    assert 'const VERSION = "1.11.17"' in _read(PANEL)
    assert "v1.11.17" in _read(PLAN)
    assert "v1.11.17" in _read(PRODUCT_PLAN)
    assert "v1.11.17" in _read(MASTER)
    assert "v1.11.17" in _read(MASTER_DB)
