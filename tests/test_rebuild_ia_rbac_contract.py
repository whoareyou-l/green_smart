...
def test_r1_version_surfaces_are_v1111():
    assert '"version": "1.11.1"' in _read(MANIFEST)
    assert 'const VERSION = "1.11.1"' in _read(PANEL)
    assert "v1.11.1" in _read(IA)
    assert "v1.11.1" in _read(UI_DOC)
    assert "v1.11.1" in _read(BACKEND_DOC)
