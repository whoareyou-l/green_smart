...
def test_v1974_ai_strategy_version_markers_and_future_shift():
    ...
    assert '"version": "1.9.80"' in manifest
    assert 'const VERSION = "1.9.80"' in panel
    assert 'v1.9.80' in panel[:200]
    assert 'EDGE_VERSION = "1.9.80"' in central
    assert '기준 버전: `v1.9.80`' in docs
