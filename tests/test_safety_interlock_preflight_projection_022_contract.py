...
def test_rs022_version_surfaces_are_aligned_to_1_12_21():
    assert '"version": "1.12.22"' in _read(MANIFEST)
    assert 'const VERSION = "1.12.22"' in _read(LEGACY_PANEL)
    assert 'REBUILD_VERSION = "1.12.22"' in _read(REBUILD_PANEL)
    for path in (DOC, INTERFACE_SPEC, PRODUCT_PLAN, LEGACY_INVENTORY):
        assert "v1.12.22" in _read(path)
...
def test_docs_specs_plan_and_inventory_record_rs022_and_next_step():
    ...
    for marker in ("Phase R4.18 — Safety/Interlock preflight projection", "Status:** `v1.12.22`에서 Safety/Interlock preflight projection 완료", "No production route removal in RS-022", "No DB migration in RS-022", "No write/mutation in RS-022"):
