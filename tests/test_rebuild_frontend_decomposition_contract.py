...
def test_r2_version_surfaces_are_v1112():
    assert '"version": "1.11.2"' in _read(MANIFEST)
    assert 'const VERSION = "1.11.2"' in _read(PANEL)
    assert "v1.11.2" in _read(PLAN)
    assert "v1.11.2" in _read(PRODUCT_PLAN)
    assert "v1.11.2" in _read(MASTER)
