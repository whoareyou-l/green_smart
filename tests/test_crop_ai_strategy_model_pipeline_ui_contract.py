...
def test_v11020_ai_strategy_first_screen_is_completed_crop_model_pipeline():
    card = _report_card()
    markers = [
    ...
    assert "작물 모델 파이프라인" in pipeline
    ordered_steps = [
...
def test_v11020_review_request_summary_is_request_only_and_uses_action_model_output():
    card = _report_card()
    summary = _section(card, "data-crop-ai-review-request-summary", "data-crop-ai-support-status-summary")
    assert "검토 요청 요약" in summary
    assert "data-crop-action-work-request" in summary
    assert "data-crop-action-model-request" in summary
    assert "data-crop-action-priority-code" in summary
    assert "targetCandidateAuthorityCode" in summary
    assert "실행 없음" in summary or "자동 실행 없음" in summary
    for forbidden in (
...
def test_v11020_interlock_and_model_status_are_support_not_primary_strategy():
    card = _report_card()
    support = _section(card, "data-crop-ai-support-status-summary", "data-crop-ai-advanced-details")
    assert "인터록/모델 운영 상태" in support
    assert "data-crop-interlock-card" in support
    assert "data-crop-interlock-approval-gate" in support
    assert "data-crop-ai-input-status" in support
    assert "data-crop-ai-ml-readiness-status" in support
    assert card.index("data-crop-ai-model-pipeline-summary") < card.index("data-crop-ai-support-status-summary")
