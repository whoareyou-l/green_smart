...
def test_r7_061_growth_save_uses_uppercase_post_and_numeric_season_path():
    ...
    save_calls = [call for call in calls if '/api/green_smart/rebuild/crop-records/' in call.get('path','')]
    assert save_calls, calls
    save = save_calls[0]
    assert save['method'] == 'POST'
    assert save['path'] == '/api/green_smart/rebuild/crop-records/7/growth-survey'
