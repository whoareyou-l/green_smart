...
def test_r0_release_version_is_v1110_everywhere():
    assert '"version": "1.11.0"' in _read(MANIFEST)
    assert 'const VERSION = "1.11.0"' in _read(PANEL)
    assert "v1.11.0" in _read(INVENTORY)
    assert "v1.11.0" in _read(RISK)
    assert "v1.11.0" in _read(PLAN)
