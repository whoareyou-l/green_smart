...
def test_v1_15_32_cached_settings_hydrate_uses_real_cards_not_compact_summary():
    text = source()
    assert 'const REBUILD_VERSION = "1.15.52"' in text
...
def test_v1_15_19_plan_documents_no_full_panel_hydrate():
    plan = PLAN.read_text()
    for marker in ['full hydrate 분해', 'panel.innerHTML = fullHtml', 'summary-card-dirty-patch', 'not-used-compact-patch', 'GitHub Release v1.15.52']:
