...
def test_legacy_panel_still_exists_but_is_not_the_rebuild_start_surface():
    ...
    # R7-115/v1.15.20 adds device/group create modals that reuse the greenhouse-create common modal grammar.
    # R7-116/v1.15.20 wires Settings users-permissions cards to real approval/permission/user role APIs.
    ...
    # v1.15.20 separates profile/logout controls and adds mobile top two-row navigation while keeping the rebuild surface bounded.
    # v1.15.20 adds mobile Settings domain shell cache helpers so settings entry avoids workspace innerHTML replacement.
    assert len(rebuild.splitlines()) < 5665
