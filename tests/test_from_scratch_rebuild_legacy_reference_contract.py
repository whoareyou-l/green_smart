...
def test_legacy_panel_still_exists_but_is_not_the_rebuild_start_surface():
    ...
    # R7-115/v1.14.97 adds device/group create modals that reuse the greenhouse-create common modal grammar.
    # R7-116/v1.14.97 wires Settings users-permissions cards to real approval/permission/user role APIs.
    ...
    # v1.14.97 adds a bounded host/root viewport-fill policy so Home Assistant custom-element shrink width does not cap domain cards.
    assert len(rebuild.splitlines()) < 4925
