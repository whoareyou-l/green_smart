...
def test_legacy_panel_still_exists_but_is_not_the_rebuild_start_surface():
    ...
    # R7-115/v1.15.11 adds device/group create modals that reuse the greenhouse-create common modal grammar.
    # R7-116/v1.15.11 wires Settings users-permissions cards to real approval/permission/user role APIs.
    ...
    # v1.15.11 separates profile/logout controls and adds mobile top two-row navigation while keeping the rebuild surface bounded.
    # v1.15.11 adds mobile two-phase placeholder/hydration so clicks can repaint before heavy panel DOM insertion.
    assert len(rebuild.splitlines()) < 5185
