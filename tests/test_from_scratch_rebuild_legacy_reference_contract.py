...
def test_legacy_panel_still_exists_but_is_not_the_rebuild_start_surface():
    ...
    # RS-003~RS-027 and R7-001 add real CBA interactions, API loading, crop_cycle cards, assignment read models, read-only projections,
    # and virtual runner contract/result adapters while keeping the rebuild surface far below the legacy panel scale.
    assert len(rebuild.splitlines()) < 720
